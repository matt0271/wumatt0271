          ...
          <div className="space-y-4 text-left">
            ...
            {showDetails && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-left p-5 bg-slate-100/50 rounded-2xl border border-slate-100 animate-in fade-in slide-in-from-top-2">
                ...
                <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">出生年月日</label><input type="date" className="w-full p-3 rounded-xl border bg-white outline-none focus:ring-2 focus:ring-teal-500" value={formData.birthDate} onChange={e=>setFilters && setFormData({...formData, birthDate:e.target.value})} /></div>
...
const App = () => {
  ...
  const fetchData = async () => { 
    try { 
      ...
      let rawLogs = Array.isArray(resLogs) ? resLogs : [];
