...
const StatusBadge = ({ status }) => {
  ...
  if (finalized) {
    ...
    return (
      <div className="w-[84px] h-[48px] flex items-center justify-center shrink-0">
        <div className={`relative inline-flex items-center justify-center ${config.rotate} opacity-80 pointer-events-none select-none`}>
          <div className={`w-[42px] h-[42px] relative flex flex-col items-center justify-center border-[2px] rounded-full font-black text-[9px] leading-none bg-white/40 backdrop-blur-sm outline outline-1 outline-offset-2 ${config.color} shadow-sm overflow-hidden`}>
            {config.icon}
  ...
  return (
    <div className="w-[84px] h-[48px] flex items-center justify-center shrink-0">
      <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold border ${currentStyle} whitespace-nowrap shadow-sm`}>
        {currentLabel}
      </span>
    </div>
  );
...
const WelcomeView = ({ userSession, records, onRefresh, setActiveMenu, isAdmin, announcements, employees }) => {
  ...
  const userWarningStatus = useMemo(() => {
    ...
    if (projectedTotal > 240 && daysLeft <= 90 && daysLeft > 0) {
      return { active: true, daysLeft, projectedTotal, overHours: projectedTotal - 240, nextAnnivStr: nextAnniv.toISOString().split('T')[0] };
    }
    return null;
    ...
    <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500 text-left font-sans relative">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden text-left">
        ...
        <div className="divide-y divide-slate-100">
          {displayAnnouncements.length > 0 ? displayAnnouncements.map(ann => {
            ...
            return (
            <div key={ann.id} onClick={() => setSelectedAnnouncement(ann)} className="p-5 sm:px-8 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 hover:bg-slate-50 transition-colors cursor-pointer group">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
          <div className="flex items-center gap-5"><div className="p-4 bg-emerald-50 text-emerald-600 rounded-2xl"><CalendarDays size={28} /></div><div><div className="flex items-center gap-2 mb-1"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">特休餘額</p>{userWarningStatus && <AlertTriangle size={14} className="text-rose-500 animate-bounce" title="即將超標" />}</div><div className="flex items-baseline gap-1"><span className={`text-3xl font-black ${userWarningStatus ? 'text-rose-600' : 'text-slate-800'}`}>{userSession.hireDate ? remainAnnual : '-'}</span><span className="text-sm font-bold text-slate-500">HR</span></div></div></div>
    ...
    <div className="space-y-8 animate-in fade-in duration-500 text-left font-sans relative">
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden text-left">
        <div className="bg-yellow-300 px-8 py-10 text-yellow-950 flex justify-between items-center text-left">
          <div><h1 className="text-2xl font-black text-yellow-950 text-left">資訊公告</h1><p className="text-sm opacity-80 italic text-yellow-900 text-left">查看公司所有最新與歷史公告</p></div><Bell size={40} className="opacity-40 text-yellow-700" />
        </div>
        <div className="divide-y divide-slate-100">
    ...
    setTimeout(() => {
      if (identifier.trim() === 'root') {
        ...
        if (password.trim() === dynamicPassword) {
          onLogin({ id: 'root', empId: 'root', name: '系統管理員', jobTitle: '最高管理員', dept: '系統維護部', hireDate: fiveYearsAgo.toISOString() });
          return;
        } else {
          setError('帳號或密碼不正確');
          setLoading(false);
...
const OvertimeView = ({ currentSerialId, onRefresh, records, employees, setNotification, userSession, availableDepts }) => {
  ...
  const [formData, setFormData] = useState({ name: userSession.name, empId: userSession.empId, dept: userSession.dept || '', category: 'regular', compensationType: 'leave', startDate: '', startHour: '', startMin: '00', endDate: '', endHour: '', endMin: '00', reason: '' });
  ...
  const calculatedCompensation = useMemo(() => {
    ...
    } else if (formData.category === 'holiday') {
      if (h <= 8) payHours = 8;
  ...
  const currentMonthOTHours = useMemo(() => {
    ...
    const targetMonth = formData.startDate.substring(0, 7);
    return records.filter(r => r.formType === '加班' && r.empId === formData.empId && r.status === 'approved' && r.startDate && r.startDate.substring(0, 7) === targetMonth).reduce((sum, r) => sum + (parseFloat(r.totalHours) || 0), 0);
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${NGROK_URL}/api/records`, { method: 'POST', headers: fetchOptions.headers, body: JSON.stringify({ ...formData, serialId: currentSerialId, formType: '加班', appType, totalHours, status: 'pending_manager', createdAt: new Date().toISOString() }) });
    ...
    <div className="space-y-8 animate-in fade-in duration-500 text-left text-slate-900">
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
        <div className={`${appType === 'pre' ? 'bg-blue-500' : 'bg-orange-500'} px-8 py-10 text-white relative transition-colors duration-500`}>
          <div className="absolute top-6 right-8 bg-white/10 backdrop-blur-md px-4 py-2 rounded-full border border-white/20 font-bold text-[11px] font-mono shadow-sm">NO.{currentSerialId}</div>
          <h1 className="text-2xl font-black">加班申請單</h1>
          <p className="mt-1 text-sm opacity-90 font-medium">{appType === 'pre' ? '事前申請' : '事後補報'}</p>
        </div>
        <form onSubmit={handleSubmit} className="p-8 space-y-8">
          {/* 表單內容省略（結構不變） */}
          <div className="grid grid-cols-2 gap-4 p-1.5 bg-slate-100 rounded-2xl">
            <button type="button" onClick={() => setAppType('pre')} className={`flex items-center justify-center gap-3 py-4 rounded-xl text-sm font-black transition-all ${appType === 'pre' ? 'bg-white text-blue-600 shadow-md scale-[1.02]' : 'text-slate-400'}`}><Timer size={20} />事前申請</button>
            <button type="button" onClick={() => setAppType('post')} className={`flex items-center justify-center gap-3 py-4 rounded-xl text-sm font-black transition-all ${appType === 'post' ? 'bg-white text-orange-600 shadow-md scale-[1.02]' : 'text-slate-400'}`}><History size={20} />事後補報</button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">員編</label><input type="text" className="w-full h-12 px-4 rounded-xl border bg-white font-mono font-bold" value={formData.empId} onChange={e=>handleEmpIdChange(e.target.value)} /></div>
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">姓名</label><input type="text" className="w-full h-12 px-4 rounded-xl border bg-white font-bold" value={formData.name} onChange={e=>handleNameChange(e.target.value)} /></div>
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">部門</label><select required className="w-full h-12 px-4 rounded-xl border bg-white font-bold" value={formData.dept} onChange={e=>setFormData({...formData, dept:e.target.value})}><option value="" disabled>請選擇部門</option>{availableDepts.map(d=><option key={d} value={d}>{d}</option>)}</select></div>
            ...
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">補償方式</label><div className="flex bg-slate-100 p-1 rounded-xl h-12"><button type="button" onClick={()=>setFormData({...formData, compensationType:'leave'})} className={`flex-1 rounded-lg text-[10px] font-black ${formData.compensationType==='leave'?(appType==='pre'?'bg-blue-500':'bg-orange-500') + ' text-white':'text-slate-500'}`}>換補休</button><button type="button" onClick={()=>setFormData({...formData, compensationType:'pay'})} className={`flex-1 rounded-lg text-[10px] font-black ${formData.compensationType==='pay'?(appType==='pre'?'bg-blue-500':'bg-orange-500') + ' text-white':'text-slate-500'}`}>計薪</button></div></div>
          </div>
          <div className="p-6 bg-slate-50 rounded-2xl border grid grid-cols-1 lg:grid-cols-12 gap-4 items-end">
            <div className="lg:col-span-4 text-left"><label className="text-xs font-bold text-slate-500 mb-2 font-black">開始時間</label><div className="flex gap-2"><input type="date" required className="flex-1 h-12 px-4 rounded-xl border font-bold bg-white" value={formData.startDate} onChange={e=>setFormData({...formData, startDate:e.target.value, endDate:e.target.value})} /><select className="h-12 w-20 rounded-xl border font-bold bg-white" value={formData.startHour} onChange={e=>setFormData({...formData, startHour:e.target.value})} required>{HOURS.map(h=><option key={h} value={h}>{h}</option>)}</select><select className="h-12 w-20 rounded-xl border font-bold bg-white" value={formData.startMin} onChange={e=>setFormData({...formData, startMin:e.target.value})} required>{MINUTES.map(m=><option key={m} value={m}>{m}</option>)}</select></div></div>
            <div className="lg:col-span-4 text-left"><label className="text-xs font-bold text-slate-500 mb-2 font-black">結束時間</label><div className="flex gap-2"><input type="date" required className="flex-1 h-12 px-4 rounded-xl border font-bold bg-white" value={formData.endDate} onChange={e=>setFormData({...formData, endDate:e.target.value})} /><select className="h-12 w-20 rounded-xl border font-bold bg-white" value={formData.endHour} onChange={e=>setFormData({...formData, endHour:e.target.value})} required>{HOURS.map(h=><option key={h} value={h}>{h}</option>)}</select><select className="h-12 w-20 rounded-xl border font-bold bg-white" value={formData.endMin} onChange={e=>setFormData({...formData, endMin:e.target.value})} required>{MINUTES.map(m=><option key={m} value={m}>{m}</option>)}</select></div></div>
            <div className={`${appType === 'pre' ? 'bg-blue-500' : 'bg-orange-500'} rounded-2xl p-3 text-white flex flex-col justify-center items-center lg:col-span-2 h-[72px] font-black`}><span className="text-[9px] uppercase opacity-70">時數</span><div className="flex items-baseline gap-1"><span className="text-xl">{totalHours || "0"}</span><span className="text-[9px]">HR</span></div></div>
            <div className="bg-slate-200 rounded-2xl p-3 text-slate-600 flex flex-col justify-center items-center lg:col-span-2 h-[72px] font-black shadow-inner"><span className="text-[9px] uppercase opacity-70 whitespace-nowrap">{formData.compensationType === 'leave' ? '預計補休' : '預計加班費'}</span><div className="flex items-baseline gap-1"><span className="text-xl text-slate-700">{formData.compensationType === 'leave' ? calculatedCompensation.leave : calculatedCompensation.payStr}</span><span className="text-[9px] text-slate-500">{formData.compensationType === 'leave' ? 'HR' : '倍時薪'}</span></div></div>
          </div>
          <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase">原因說明</label><textarea required rows="2" placeholder="請描述加班具體工作內容..." className="w-full p-4 rounded-xl border bg-white font-bold text-slate-900 outline-none focus:ring-4 focus:ring-slate-100" value={formData.reason} onChange={e=>setFormData({...formData, reason:e.target.value})} /></div>
          {isOverLimit && <div className="p-4 bg-rose-50 border border-rose-200 rounded-2xl flex items-start gap-3 text-rose-700 text-sm font-bold shadow-sm animate-in fade-in"><AlertTriangle size={20} className="shrink-0 text-rose-500" /><div>送出限制：當月加班時數將超過 46 小時法定上限！<div className="text-xs font-medium mt-1 text-rose-600">當月已核准：{currentMonthOTHours} 小時 + 本次申請：{totalHours || 0} 小時 = 預計 {projectedTotalHours} 小時</div></div></div>}
          <button disabled={totalHours <= 0 || submitting || isOverLimit} type="submit" className={`w-full py-4 rounded-2xl font-black text-white shadow-xl transition-all active:scale-[0.98] ${totalHours <= 0 || submitting || isOverLimit ? 'bg-slate-300' : (appType === 'pre' ? 'bg-blue-500 hover:bg-blue-600' : 'bg-orange-500 hover:bg-orange-600')}`}>{submitting ? '提交中...' : `送出加班申請 (${appType === 'pre' ? '事前' : '事後'})`}</button>
        </form>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
        ...
        {recentSubmissions.length > 0 ? (
          <div className="space-y-2">
            {/* Header for Overtime Records */}
            <div className="hidden md:grid md:grid-cols-[1.2fr_1fr_0.6fr_1fr_2.5fr_0.6fr_1.5fr] gap-4 px-6 py-3 bg-slate-50 rounded-xl mb-2 text-[10px] font-black text-slate-400 uppercase tracking-widest border border-slate-100">
              <div>單號</div>
              <div>部門</div>
              <div>類型</div>
              <div>類別</div>
              <div>時間範圍</div>
              <div>時數</div>
              <div className="text-right pr-4">狀態 / 操作</div>
            </div>
            {recentSubmissions.map(r => (
              <div key={r.id} className="p-4 md:px-6 md:py-3 rounded-2xl bg-white border border-slate-100 hover:border-blue-200 hover:shadow-sm transition-all text-slate-900 group">
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-[1.2fr_1fr_0.6fr_1fr_2.5fr_0.6fr_1.5fr] gap-2 md:gap-4 items-center w-full">
                  <div><p className="md:hidden text-[10px] font-black text-slate-300 uppercase">單號</p><p className="font-mono font-bold text-slate-500 text-xs truncate">{r.serialId}</p></div>
                  <div><p className="md:hidden text-[10px] font-black text-slate-300 uppercase">部門</p><p className="font-bold text-slate-700 text-xs truncate">{r.dept || '未設定'}</p></div>
                  <div><p className="md:hidden text-[10px] font-black text-slate-300 uppercase">類型</p><p className={`font-black text-[11px] ${r.appType === 'pre' ? 'text-blue-600' : 'text-orange-600'}`}>{r.appType === 'pre' ? '事前' : '事後'}</p></div>
                  <div><p className="md:hidden text-[10px] font-black text-slate-300 uppercase">類別</p><p className="font-bold text-[11px] text-slate-600 truncate">{OT_CATEGORIES.find(c => c.id === r.category)?.label || '未設定'}</p></div>
                  <div>
                    <p className="md:hidden text-[10px] font-black text-slate-300 uppercase">時間</p>
                    <p className="font-bold text-[11px] text-slate-600 truncate whitespace-nowrap">
                      {r.startDate === r.endDate ? `${r.startDate} ${r.startHour}:${r.startMin}~${r.endHour}:${r.endMin}` : `${r.startDate} ${r.startHour}:${r.startMin} ~ ${r.endDate} ${r.endHour}:${r.endMin}`}
                    </p>
                  </div>
                  <div><p className="md:hidden text-[10px] font-black text-slate-300 uppercase">時數</p><p className="font-black text-xs">{r.totalHours}H</p></div>
                  <div className="flex justify-end items-center gap-2 col-span-2 sm:col-span-3 md:col-span-1">
                    <StatusBadge status={r.status} />
                    {['pending', 'pending_manager'].includes(r.status) && <button onClick={() => setWithdrawTarget(r)} className="p-2 text-rose-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="刪除單據"><Trash2 size={14}/></button>}
                    {r.status === 'approved' && <button onClick={() => setCancelTarget(r)} className="px-2 py-1 flex items-center gap-1 text-[10px] font-bold text-slate-500 bg-slate-50 hover:bg-slate-700 hover:text-white rounded-lg transition-colors" title="抽單(撤銷)"><Undo2 size={12}/> 撤銷</button>}
                  </div>
                </div>
              </div>
            ))}
...
const LeaveApplyView = ({ currentSerialId, onRefresh, employees, setNotification, userSession, records, availableDepts }) => {
  ...
  const [formData, setFormData] = useState({ name: userSession.name, empId: userSession.empId, dept: userSession.dept || '', jobTitle: userSession.jobTitle || '', substitute: '', category: 'annual', startDate: '', startHour: '', startMin: '00', endDate: '', endHour: '', endMin: '00', reason: '', attachmentName: '', attachmentData: null });
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      if (file.size > 5 * 1024 * 1024) { setNotification({ type: 'error', text: '檔案大小不能超過 5MB' }); e.target.value = ''; return; }
      const reader = new FileReader();
      reader.onloadend = () => { setFormData(prev => ({ ...prev, attachmentName: file.name, attachmentData: reader.result })); };
  ...
  const totalHours = useMemo(() => {
    ...
    while (currentDay <= end) {
      ...
      if (currentDay.getDay() !== 0 && currentDay.getDay() !== 6 && !holidays.includes(dStr)) {
        const workStart = new Date(currentDay).setHours(9, 0, 0, 0);
        const workEnd = new Date(currentDay).setHours(18, 0, 0, 0);
        const oStart = Math.max(start.getTime(), workStart);
        const oEnd = Math.min(end.getTime(), workEnd);
        if (oEnd > oStart) {
          let daily = oEnd - oStart;
          const lStart = new Date(currentDay).setHours(12, 30, 0, 0);
          const lEnd = new Date(currentDay).setHours(13, 30, 0, 0);
          const loStart = Math.max(oStart, lStart), loEnd = Math.min(oEnd, lEnd);
          if (loEnd > loStart) daily -= (loEnd - loStart);
          totalValidMs += daily;
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.category === 'annual' && totalHours > stats.remainAnnual) { setNotification({ type: 'error', text: `特休餘額不足！` }); setSubmitting(false); return; }
      if (formData.category === 'comp' && totalHours > stats.remainComp) { setNotification({ type: 'error', text: `補休餘額不足！` }); setSubmitting(false); return; }
    ...
    <div className="space-y-8 animate-in fade-in duration-500 text-left text-slate-900 font-sans">
      {withdrawTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl">
            <AlertTriangle size={48} className="text-rose-500 mx-auto mb-4" />
            <h3 className="text-xl font-black mb-2">確定要刪除申請？</h3>
            <p className="text-sm text-slate-500 mb-8 font-bold">單號：{withdrawTarget.serialId}</p>
            <div className="flex gap-3"><button onClick={() => setWithdrawTarget(null)} className="flex-1 py-3 font-bold bg-slate-100 rounded-xl">取消</button><button onClick={async () => { try { await fetch(`${NGROK_URL}/api/records/${withdrawTarget.id}`, { method: 'DELETE', headers: fetchOptions.headers }); setNotification({ type: 'success', text: '已刪除' }); setWithdrawTarget(null); onRefresh(); } catch(err) { setNotification({ type: 'error', text: '失敗' }); } }} className="flex-1 py-3 font-black text-white bg-rose-500 rounded-xl">確認刪除</button></div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
        <div className="bg-emerald-500 px-8 py-10 text-white relative flex justify-between items-center">
          <div><h1 className="text-2xl font-black">請假申請單</h1><p className="text-sm opacity-80">填寫申請時段與具體理由</p></div>
          <div className="bg-white/10 backdrop-blur-md px-4 py-2 rounded-full border border-white/20 font-mono font-bold text-xs shadow-sm text-white">NO.{currentSerialId}</div>
        </div>
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">員編</label><input type="text" className="w-full h-12 px-4 rounded-xl border bg-white font-mono font-bold" value={formData.empId} onChange={e=>handleEmpIdChange(e.target.value)} /></div>
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">姓名</label><input type="text" className="w-full h-12 px-4 rounded-xl border bg-white font-bold" value={formData.name} onChange={e=>handleNameChange(e.target.value)} /></div>
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">部門</label><select required className="w-full h-12 px-4 rounded-xl border bg-white font-bold" value={formData.dept} onChange={e=>setFormData({...formData, dept:e.target.value, substitute: ''})}><option value="" disabled>請選擇部門</option>{availableDepts.map(d=><option key={d} value={d}>{d}</option>)}</select></div>
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">職稱</label><input type="text" className="w-full h-12 px-4 rounded-xl border bg-white font-bold" value={formData.jobTitle} onChange={e=>setFormData({...formData, jobTitle:e.target.value})} /></div>
            ...
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">代理人</label><select required className="w-full h-12 px-4 rounded-xl border bg-white font-bold" value={formData.substitute} onChange={e=>setFormData({...formData, substitute:e.target.value})}><option value="" disabled>請選擇代理人</option>{availableSubstitutes.map(emp => (<option key={emp.empId} value={emp.name}>{emp.name}</option>))}</select></div>
          </div>
          <div className="p-6 bg-slate-50 rounded-2xl border grid grid-cols-1 lg:grid-cols-12 gap-4 items-end">
            <div className="lg:col-span-5 text-left"><label className="text-xs font-bold text-emerald-600 mb-2 font-black">開始時間</label><div className="flex gap-2"><input type="date" required className="flex-1 h-12 px-4 rounded-xl border bg-white font-bold" value={formData.startDate} onChange={e=>setFormData({...formData, startDate:e.target.value, endDate:e.target.value})} /><select className="h-12 w-20 rounded-xl border font-bold bg-white" value={formData.startHour} onChange={e=>setFormData({...formData, startHour:e.target.value})} required>{HOURS.map(h=><option key={h} value={h}>{h}</option>)}</select><select className="h-12 w-20 rounded-xl border font-bold bg-white" value={formData.startMin} onChange={e=>setFormData({...formData, startMin:e.target.value})} required>{MINUTES.map(m=><option key={m} value={m}>{m}</option>)}</select></div></div>
            <div className="lg:col-span-5 text-left"><label className="text-xs font-bold text-rose-500 mb-2 font-black">結束時間</label><div className="flex gap-2"><input type="date" required className="flex-1 h-12 px-4 rounded-xl border bg-white font-bold" value={formData.endDate} onChange={e=>setFormData({...formData, endDate:e.target.value})} /><select className="h-12 w-20 rounded-xl border font-bold bg-white" value={formData.endHour} onChange={e=>setFormData({...formData, endHour:e.target.value})} required>{HOURS.map(h=><option key={h} value={h}>{h}</option>)}</select><select className="h-12 w-20 rounded-xl border font-bold bg-white" value={formData.endMin} onChange={e=>setFormData({...formData, endMin:e.target.value})} required>{MINUTES.map(m=><option key={m} value={m}>{m}</option>)}</select></div></div>
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
        ...
        {recentSubmissions.length > 0 ? (
          <div className="space-y-2">
            {/* Header for Leave Records */}
            <div className="hidden md:grid md:grid-cols-[1.2fr_1fr_1.2fr_2.5fr_0.6fr_1.5fr] gap-4 px-6 py-3 bg-slate-50 rounded-xl mb-2 text-[10px] font-black text-slate-400 uppercase tracking-widest border border-slate-100">
              <div>單號 / 附件</div>
              <div>部門</div>
              <div>假別類型</div>
              <div>請假時段範圍</div>
            ...
            {recentSubmissions.map(r => (
              <div key={r.id} className="p-4 md:px-6 md:py-3 rounded-2xl bg-white border border-slate-100 hover:border-emerald-200 hover:shadow-sm transition-all text-slate-900 group">
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-[1.2fr_1fr_1.2fr_2.5fr_0.6fr_1.5fr] gap-2 md:gap-4 items-center w-full">
                  <div>
                    <p className="md:hidden text-[10px] font-black text-slate-300 uppercase">單號</p>
                    <div className="flex flex-col">
                      <span className="font-mono font-bold text-slate-500 text-xs truncate">{r.serialId}</span>
                      {r.attachmentName && <a href={r.attachmentData} download={r.attachmentName} className="inline-flex items-center gap-1 text-[9px] font-bold text-sky-600 mt-0.5"><Paperclip size={10} /> 附件</a>}
                    </div>
                  </div>
                  <div><p className="md:hidden text-[10px] font-black text-slate-300 uppercase">部門</p><p className="font-bold text-slate-700 text-xs truncate">{r.dept || '未設定'}</p></div>
                  <div><p className="md:hidden text-[10px] font-black text-slate-300 uppercase">假別</p><p className="font-bold text-[11px] text-slate-600 truncate">{LEAVE_CATEGORIES.find(c => c.id === r.category)?.label || '未設定'}</p></div>
                  <div>
                    <p className="md:hidden text-[10px] font-black text-slate-300 uppercase">時間</p>
                    <p className="font-bold text-[11px] text-slate-600 truncate whitespace-nowrap">
                      {r.startDate === r.endDate ? `${r.startDate} ${r.startHour}:${r.startMin}~${r.endHour}:${r.endMin}` : `${r.startDate} ${r.startHour}:${r.startMin} ~ ${r.endDate} ${r.endHour}:${r.endMin}`}
                  ...
                  <div className="flex justify-end items-center gap-2 col-span-2 sm:col-span-3 md:col-span-1">
                    <StatusBadge status={r.status} />
                    {['pending', 'pending_substitute', 'pending_manager'].includes(r.status) && <button onClick={() => setWithdrawTarget(r)} className="p-2 text-rose-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="刪除單據"><Trash2 size={14}/></button>}
                    {r.status === 'approved' && <button onClick={() => setCancelTarget(r)} className="px-2 py-1 flex items-center gap-1 text-[10px] font-bold text-slate-500 bg-slate-50 hover:bg-slate-700 hover:text-white rounded-lg transition-colors" title="銷假(抽單)"><Undo2 size={12}/> 銷假</button>}
                  </div>
                </div>
              </div>
            ))}
    ...
    <div className="space-y-6 animate-in fade-in duration-500 text-left text-slate-900 font-sans">
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden text-left">
        <div className="bg-fuchsia-500 px-8 py-10 text-white flex justify-between items-center">
          <div><h1 className="text-2xl font-black">申請單據查詢</h1><p className="text-sm opacity-90 italic">設定條件查詢您的歷史單據</p></div><Search size={40} className="opacity-30" />
        </div>
        <form onSubmit={handleSearch} className="p-8 border-b border-slate-100 bg-slate-50/50 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">單據類型</label><select className="w-full h-12 px-4 rounded-xl border bg-white font-bold" value={filters.formType} onChange={e => setFilters({...filters, formType: e.target.value})}><option value="">全部</option><option value="加班">加班申請</option><option value="請假">請假申請</option></select></div>
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">單號包含</label><input type="text" placeholder="例如: OT001" className="w-full h-12 px-4 rounded-xl border bg-white font-bold" value={filters.serialId} onChange={e => setFilters({...filters, serialId: e.target.value})} /></div>
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">簽核狀態</label><select className="w-full h-12 px-4 rounded-xl border bg-white font-bold" value={filters.status} onChange={e => setFilters({...filters, status: e.target.value})}><option value="">全部</option><option value="pending_substitute">待代理確認</option><option value="pending_manager">待主管簽核</option><option value="approved">已核准</option><option value="rejected">已駁回</option><option value="canceled">已撤銷(銷假)</option></select></div>
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">起始日期</label><input type="date" className="w-full h-12 px-4 rounded-xl border bg-white font-bold" value={filters.startDate} onChange={e => setFilters({...filters, startDate: e.target.value})} /></div>
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">結束日期</label><input type="date" className="w-full h-12 px-4 rounded-xl border bg-white font-bold" value={filters.endDate} onChange={e => setFilters({...filters, endDate: e.target.value})} /></div>
          </div>
          <div className="flex gap-3 justify-end pt-2">
            <button type="button" onClick={handleReset} className="px-6 py-3 rounded-xl font-bold text-slate-500 bg-slate-200">清除重設</button>
            <button type="submit" className="px-8 py-3 rounded-xl font-black text-white bg-fuchsia-500 shadow-md flex items-center gap-2"><Search size={18}/> 執行查詢</button>
          </div>
        </form>
        <div className="p-8 space-y-2">
          {!hasSearched ? (
            <div className="py-24 text-center text-slate-400 font-bold flex flex-col items-center gap-3"><Search size={48} className="opacity-20 mb-2 text-fuchsia-500" /><p>請設定上方查詢條件，並點擊「執行查詢」</p></div>
          ) : searchResults.length > 0 ? (
            <>
              {/* Inquiry List Header */}
              <div className="hidden md:grid md:grid-cols-[0.8fr_1.2fr_1fr_2.5fr_0.6fr_1.2fr] gap-4 px-6 py-3 bg-slate-50 rounded-xl mb-2 text-[10px] font-black text-slate-400 uppercase tracking-widest border border-slate-100">
                <div>單據類型</div>
                <div>單號</div>
                <div>部門</div>
                <div>時間範圍</div>
                <div>時數</div>
                <div className="text-right pr-4">狀態</div>
              </div>
              {searchResults.map(r => (
                <div key={r.id} className="p-4 md:px-6 md:py-3 rounded-2xl bg-white border border-slate-100 hover:border-fuchsia-200 transition-all text-slate-900 shadow-sm group">
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-[0.8fr_1.2fr_1fr_2.5fr_0.6fr_1.2fr] gap-2 md:gap-4 items-center w-full">
                    <div><span className={`px-2 py-0.5 rounded-lg text-[10px] font-black ${r.formType === '請假' ? 'bg-emerald-50 text-emerald-700' : 'bg-blue-50 text-blue-700'}`}>{r.formType === '請假' ? '請假' : '加班'}</span></div>
                    <div className="flex flex-col">
                      <span className="font-mono font-bold text-fuchsia-600 text-xs truncate">{r.serialId}</span>
                      {r.attachmentName && <span className="text-[9px] text-slate-400 font-bold">含附件</span>}
                    </div>
                    <div><p className="font-bold text-slate-700 text-xs truncate">{r.dept || '未設定'}</p></div>
                    <div><p className="font-bold text-[11px] text-slate-600 truncate whitespace-nowrap">{r.startDate === r.endDate ? `${r.startDate} ${r.startHour}:${r.startMin}~${r.endHour}:${r.endMin}` : `${r.startDate} ${r.startHour}:${r.startMin} ~ ${r.endDate} ${r.endHour}:${r.endMin}`}</p></div>
...
const ChangePasswordView = ({ userSession, setNotification, onLogout, onRefresh }) => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      if (res.ok) { setNotification({ type: 'success', text: '密碼更新成功，即將登出...' }); onRefresh(); setTimeout(() => onLogout(), 2000); }
    ...
    <div className="space-y-6 animate-in fade-in duration-500 text-left font-sans text-slate-900">
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
        <div className="bg-slate-700 px-8 py-10 text-white flex justify-between items-center">
          <div><h1 className="text-2xl font-black">帳號安全設定</h1><p className="text-sm opacity-90 italic">變更後將強制登出以確認生效</p></div><KeyRound size={40} className="opacity-30" />
        </div>
        <form onSubmit={handleUpdate} className="p-10 space-y-8 max-w-lg mx-auto py-16">
          ...
          <button disabled={loading} className="w-full py-5 rounded-2xl font-black text-white bg-slate-700 shadow-xl active:scale-95 flex items-center justify-center gap-3 transition-all">{loading ? <Loader2 size={20} className="animate-spin" /> : <CheckCircle size={20} />} 儲存變更</button>
        </form>
      </div>
    </div>
  );
...
const SubstituteView = ({ records, onRefresh, setNotification, userSession }) => {
  ...
  const handleUpdate = async (status) => {
    ...
    try {
      ...
      setNotification({ type: 'success', text: '完成' });
      setSelectedId(null); setOpinion(''); onRefresh();
    } catch (err) { setNotification({ type: 'error', text: '錯誤' }); } finally { setUpdating(false); }
  };
    ...
    <div className="space-y-6 pb-20 text-left font-sans text-slate-900">
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
        <div className="bg-amber-500 p-8 text-white flex justify-between items-center text-left">
          <div className="space-y-1"><div className="flex items-center gap-2 mb-1"><span className="px-2 py-0.5 bg-white/20 rounded text-[10px] font-bold uppercase tracking-wider text-white">待確認</span></div><h1 className="text-2xl font-black text-white">代理確認中心</h1><p className="text-sm opacity-90 font-medium italic">確認同仁指定您為代理人的申請</p></div>
        ...
        <div className="p-8 space-y-2">
          {pendingRecords.length > 0 ? (
            <>
              ...
              <div className="hidden md:grid md:grid-cols-[auto_1.2fr_1fr_2.5fr_0.6fr_1.5fr] gap-4 px-6 py-3 bg-slate-50 rounded-xl mb-2 text-[10px] font-black text-slate-400 uppercase tracking-widest border border-slate-100">
              ...
              {pendingRecords.map(r => (
                <div key={r.id} onClick={() => setSelectedId(r.id)} className={`p-4 md:px-6 md:py-3 rounded-2xl border transition-all cursor-pointer ${selectedId === r.id ? 'bg-amber-50 border-amber-300 ring-2 ring-inset ring-amber-200' : 'bg-white hover:border-amber-200 border-slate-100 shadow-sm'}`}>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-[auto_1.2fr_1fr_2.5fr_0.6fr_1.5fr] gap-2 md:gap-4 items-center w-full">
                    <div className="flex items-center justify-center"><div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${selectedId === r.id ? 'border-amber-600 bg-amber-600' : 'border-slate-200'}`}>{selectedId === r.id && <div className="w-1.5 h-1.5 rounded-full bg-white" />}</div></div>
                    <div><p className="font-black text-slate-800 text-xs truncate">{r.name}</p><p className="text-[10px] text-slate-400 font-bold">{r.dept}</p></div>
                    <div><p className="font-bold text-[11px] text-slate-600 truncate">{LEAVE_CATEGORIES.find(c => c.id === r.category)?.label || '請假'}</p><p className="font-mono text-[9px] text-slate-400">{r.serialId}</p></div>
                    <div><p className="font-bold text-[11px] text-slate-600 truncate whitespace-nowrap">{r.startDate === r.endDate ? `${r.startDate} ${r.startHour}:${r.startMin}~${r.endHour}:${r.endMin}` : `${r.startDate} ${r.startHour}:${r.startMin} ~ ${r.endDate} ${r.endHour}:${r.endMin}`}</p></div>
                    <div><p className="font-black text-xs">{r.totalHours}H</p></div>
                    <div className="flex justify-end"><StatusBadge status={r.status} /></div>
                  </div>
      ...
      {selectedId && (
        <div className="bg-white rounded-3xl shadow-xl border border-amber-200 p-8 flex flex-col md:flex-row gap-8 animate-in slide-in-from-bottom-2">
          <div className="flex-1 space-y-4"><div className="flex items-center gap-2 text-amber-600 font-black text-sm"><MessageSquare size={18} /> 代理人回覆 <span className="text-rose-400 font-bold text-[10px]">* 拒絕為必填</span></div><textarea placeholder="填寫您拒絕或同意的意見..." className="w-full p-4 rounded-2xl border bg-slate-50 outline-none text-sm font-bold" value={opinion} onChange={(e) => setOpinion(e.target.value)} /></div>
          <div className="w-full md:w-72 flex flex-col justify-end gap-3">
            <p className="text-[10px] font-black text-slate-400 uppercase">選取單據：<span className="text-amber-600">{selectedRecord?.serialId}</span></p>
            <div className="grid grid-cols-2 gap-3"><button disabled={updating} onClick={() => handleUpdate('rejected')} className="flex flex-col items-center justify-center gap-2 py-3 bg-rose-50 text-rose-600 rounded-2xl border border-rose-100 hover:bg-rose-600 hover:text-white transition-all text-xs font-black"><XCircle size={20}/>拒絕代理</button><button disabled={updating} onClick={() => handleUpdate('pending_manager')} className="flex flex-col items-center justify-center gap-2 py-3 bg-emerald-50 text-emerald-600 rounded-2xl border border-emerald-100 hover:bg-emerald-600 hover:text-white transition-all text-xs font-black"><CheckCircle2 size={20}/>同意代理</button></div>
          </div>
        </div>
      )}
    </div>
...
const ApprovalView = ({ records, onRefresh, setNotification, userSession, employees }) => {
  ...
  const handleUpdate = async (status) => {
    if (!selectedId) return;
    if (status === 'rejected' && !opinion.trim()) return setNotification({ type: 'error', text: '原因必填' });
    setUpdating(true);
    try {
      await fetch(`${NGROK_URL}/api/records/${selectedId}/status`, { method: 'PUT', headers: fetchOptions.headers, body: JSON.stringify({ status, opinion }) });
      setNotification({ type: 'success', text: '完成' });
      setSelectedId(null); setOpinion(''); onRefresh();
    } catch (err) { setNotification({ type: 'error', text: '異常' }); } finally { setUpdating(false); }
  };
  const selectedRecord = useMemo(() => pendingRecords.find(r => r.id === selectedId), [pendingRecords, selectedId]);
    ...
    <div className="space-y-6 pb-20 text-left font-sans text-slate-900">
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden text-left">
        <div className="bg-indigo-600 p-8 text-white flex justify-between items-center text-left">
          <div className="space-y-1"><div className="flex items-center gap-2 mb-1"><span className="px-2 py-0.5 bg-white/20 rounded text-[10px] font-bold uppercase tracking-wider text-white">審核名單</span></div><h1 className="text-2xl font-black text-white">主管審核中心</h1><p className="text-sm opacity-90 font-medium italic">審核員工提交之申請單</p></div><ShieldCheck size={40} className="opacity-40 text-white" />
        </div>
        <div className="p-8 space-y-2">
          {pendingRecords.length > 0 ? (
            <>
              {/* Approval Header */}
              <div className="hidden md:grid md:grid-cols-[auto_1fr_1fr_1.2fr_2.5fr_0.5fr_1.2fr] gap-4 px-6 py-3 bg-slate-50 rounded-xl mb-2 text-[10px] font-black text-slate-400 uppercase tracking-widest border border-slate-100">
                <div className="w-5"></div>
                <div>申請人</div>
                <div>單號</div>
                <div>類型 / 項目</div>
              ...
              {pendingRecords.map(r => (
                <div key={r.id} onClick={() => setSelectedId(r.id)} className={`p-4 md:px-6 md:py-3 rounded-2xl border transition-all cursor-pointer ${selectedId === r.id ? 'bg-indigo-50 border-indigo-300 ring-2 ring-inset ring-indigo-200' : 'bg-white hover:border-indigo-200 border-slate-100 shadow-sm'}`}>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-[auto_1fr_1fr_1.2fr_2.5fr_0.5fr_1.2fr] gap-2 md:gap-4 items-center w-full">
                    <div className="flex items-center justify-center"><div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${selectedId === r.id ? 'border-indigo-600 bg-indigo-600' : 'border-slate-200'}`}>{selectedId === r.id && <div className="w-1.5 h-1.5 rounded-full bg-white" />}</div></div>
                    <div><p className="font-black text-slate-800 text-xs truncate">{r.name}</p><p className="text-[10px] text-slate-400 font-bold">{r.dept}</p></div>
                    <div><p className="font-mono text-xs text-slate-500">{r.serialId}</p></div>
                    <div><span className={`px-2 py-0.5 rounded-lg text-[10px] font-black ${r.formType === '請假' ? 'bg-emerald-50 text-emerald-700' : 'bg-blue-50 text-blue-700'}`}>{r.formType} / {r.formType === '請假' ? (LEAVE_CATEGORIES.find(c => c.id === r.category)?.label || '未設定') : (r.compensationType === 'leave' ? '換補休' : '計薪')}</span></div>
                    <div><p className="font-bold text-[11px] text-slate-600 truncate whitespace-nowrap">{r.startDate === r.endDate ? `${r.startDate} ${r.startHour}:${r.startMin}~${r.endHour}:${r.endMin}` : `${r.startDate} ${r.startHour}:${r.startMin} ~ ${r.endDate} ${r.endHour}:${r.endMin}`}</p></div>
                    <div><p className="font-black text-xs">{r.totalHours}H</p></div>
                    <div className="flex justify-end"><StatusBadge status={r.status} /></div>
                  </div>
          ...
          ) : <div className="py-12 text-center text-slate-300 italic font-bold">目前無待簽核單據</div>}
        </div>
      </div>
      {selectedId && (
        <div className="bg-white rounded-3xl shadow-xl border border-indigo-200 p-8 flex flex-col lg:flex-row gap-8 animate-in slide-in-from-top-4 duration-300">
          <div className="flex-1 flex flex-col space-y-4"><div className="flex items-center gap-2 text-indigo-600 font-black text-sm"><MessageSquare size={18} /> 主管簽核意見 <span className="text-rose-400 font-bold text-[10px]">* 駁回為必填</span></div><textarea placeholder="填寫簽核意見..." className="w-full p-4 rounded-2xl border bg-slate-50 outline-none text-sm font-bold flex-1 min-h-[100px]" value={opinion} onChange={(e) => setOpinion(e.target.value)} /></div>
          <div className="w-full lg:w-72 flex flex-col justify-end gap-3"><p className="text-[10px] font-black text-slate-400 uppercase">選取：<span className="text-indigo-600">{selectedRecord?.serialId}</span></p><div className="grid grid-cols-2 gap-3"><button disabled={updating} onClick={() => handleUpdate('rejected')} className="flex flex-col items-center justify-center gap-2 py-4 bg-rose-50 text-rose-600 rounded-2xl border border-rose-100 hover:bg-rose-600 hover:text-white transition-all text-xs font-black"><XCircle size={24}/>駁回</button><button disabled={updating} onClick={() => handleUpdate('approved')} className="flex flex-col items-center justify-center gap-2 py-4 bg-emerald-50 text-emerald-600 rounded-2xl border border-emerald-100 hover:bg-emerald-600 hover:text-white transition-all text-xs font-black"><CheckCircle2 size={24}/>核准</button></div></div>
        </div>
      )}
    </div>
  );
      ...
      {pwdTarget && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
            ...
            <h3 className="text-xl font-black mb-1">重設密碼？</h3>
            <p className="text-xs text-slate-400 mb-8 font-bold">為 {pwdTarget.name} 還原為員編密碼</p>
            <div className="flex gap-3"><button onClick={()=>setPwdTarget(null)} className="flex-1 py-3 font-bold bg-slate-100 rounded-xl">取消</button><button onClick={() => { fetch(`${NGROK_URL}/api/employees/${pwdTarget.id}`, { method: 'PUT', headers: fetchOptions.headers, body: JSON.stringify({ ...pwdTarget, password: pwdTarget.empId }) }).then(onRefresh); setPwdTarget(null); }} className="flex-1 py-3 font-black text-white bg-teal-600 rounded-xl">確認</button></div>
          </div>
        </div>
      )}
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden text-left">
        <div className="bg-teal-600 p-8 text-white flex justify-between items-center text-left">
          <div><h1 className="text-2xl font-black text-white">人員管理中心</h1><p className="text-sm opacity-90 italic text-white">維護同仁資料與 Excel 工具</p></div><Users size={40} className="opacity-40" />
        </div>
        <div className="px-8 pt-6 flex gap-3">
          <button onClick={handleExport} className="flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl text-xs font-bold border border-emerald-100"><FileSpreadsheet size={16}/> 匯出 Excel</button>
          <button onClick={() => fileInputRef.current.click()} className="flex items-center gap-2 px-4 py-2 bg-sky-50 text-sky-600 rounded-xl text-xs font-bold border border-sky-100"><Upload size={16}/> 匯入 Excel</button>
        ...
        <form onSubmit={async (e) => {
          ...
          try {
            await fetch(url, { method: editingId ? 'PUT' : 'POST', headers: fetchOptions.headers, body: JSON.stringify(payload) });
            onRefresh(); setEditingId(null); setFormData({name:'',empId:'',jobTitle:'',dept:'', gender:'', birthDate:'', hireDate:''}); setNotification({ type: 'success', text: '更新成功' });
          } catch (err) { setNotification({ type: 'error', text: '更新失敗' }); }
        }} className="p-8 space-y-6 text-left">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <input type="text" placeholder="員編" required className="p-3 rounded-xl border bg-slate-50 outline-none" value={formData.empId} onChange={e=>setFormData({...formData, empId:e.target.value})} />
            <input type="text" placeholder="姓名" required className="p-3 rounded-xl border bg-slate-50 outline-none" value={formData.name} onChange={e=>setFormData({...formData, name:e.target.value})} />
            <input type="text" list="titles-list" placeholder="職稱" required className="p-3 rounded-xl border bg-slate-50 outline-none" value={formData.jobTitle} onChange={e=>setFormData({...formData, jobTitle:e.target.value})} /><datalist id="titles-list">{availableTitles.map(t=><option key={t} value={t} />)}</datalist>
            <select required className="p-3 rounded-xl border bg-slate-50 outline-none" value={formData.dept} onChange={e => { if(e.target.value === '__custom__') { setIsCustomDept(true); setFormData({...formData, dept:''}); } else { setFormData({...formData, dept:e.target.value}); } }}><option value="" disabled>請選擇單位</option>{availableDepts.map(d=><option key={d} value={d}>{d}</option>)}<option value="__custom__">+ 新增單位</option></select>
          </div>
          {isCustomDept && <input type="text" placeholder="輸入新單位..." className="w-full p-3 rounded-xl border" value={formData.dept} onChange={e=>setFormData({...formData, dept:e.target.value})} onBlur={()=>{if(!formData.dept)setIsCustomDept(false)}} />}
          <div className="flex gap-4 pt-2">
            <button type="submit" className="flex-1 py-4 bg-teal-600 text-white rounded-2xl font-black">{editingId ? '更新資料' : '新增人員'}</button>
            <button type="button" onClick={() => { setEditingId(null); setFormData({name:'',empId:'',jobTitle:'',dept:'', gender:'', birthDate:'', hireDate:''}); }} className="w-1/4 py-4 bg-slate-100 text-slate-600 rounded-2xl font-black">取消</button>
          </div>
        </form>
        <div className="overflow-x-auto border-t text-left">
          <table className="w-full text-sm text-left text-slate-900">
            <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b"><tr><th className="px-8 py-4">員編</th><th className="px-4 py-4">姓名</th><th className="px-4 py-4">職稱 / 單位</th><th className="px-4 py-4">登入密碼</th><th className="px-8 py-4 text-right">操作</th></tr></thead>
            <tbody className="divide-y divide-slate-100">
              {filteredEmployees.map(emp => (
                <tr key={emp.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-5 text-left"><div className="flex items-center gap-3">{(emp.password && emp.password !== emp.empId) && (<span className="px-2 py-0.5 rounded-lg text-[9px] font-mono font-bold bg-emerald-100 text-emerald-700">已自訂</span>)}<button onClick={()=>setPwdTarget(emp)} className="text-[10px] font-black text-slate-500 hover:text-slate-800 flex items-center gap-1 transition-colors"><RotateCcw size={12}/>還原</button></div></td>
                  <td className="px-8 py-5 text-right flex justify-end gap-2 text-slate-900">
                    <button onClick={()=>{ setEditingId(emp.id); setFormData({ ...emp, gender: emp.gender || '', birthDate: emp.birthDate ? emp.birthDate.split('T')[0] : '', hireDate: emp.hireDate ? emp.hireDate.split('T')[0] : '' }); window.scrollTo({top:0,behavior:'smooth'}); }} className="p-2 text-slate-300 hover:text-slate-600"><Edit2 size={16} /></button>
...
const AnnouncementManagement = ({ announcements, setAnnouncements, setNotification }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.title.trim()) return setNotification({ type: 'error', text: '標題必填' });
  ...
  const handleDelete = (id) => { if(window.confirm('確定刪除？')) { setAnnouncements(prev => prev.filter(a => a.id !== id)); setNotification({ type: 'success', text: '已刪除' }); } };

  return (
    <div className="space-y-8 animate-in fade-in duration-500 text-left font-sans text-slate-900">
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden text-left">
        <div className="bg-rose-500 p-8 text-white flex justify-between items-center text-left">
          <div><h1 className="text-2xl font-black">公告維護中心</h1><p className="text-sm opacity-90 italic">發布與管理首頁的系統公告資訊</p></div><Megaphone size={40} className="opacity-40" />
        </div>
        <form onSubmit={handleSubmit} className="p-8 space-y-6 text-left border-b border-slate-100 bg-slate-50/30">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            <div className="md:col-span-2 space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">公告標題</label><input type="text" placeholder="輸入標題..." required className="w-full p-3 rounded-xl border bg-white font-bold" value={formData.title} onChange={e=>setFormData({...formData, title:e.target.value})} /></div>
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">公告類型</label><select className="w-full p-3 rounded-xl border bg-white font-bold" value={formData.type} onChange={e=>setFormData({...formData, type:e.target.value})}>{ANNOUNCEMENT_TYPES.map(t => (<option key={t.id} value={t.id}>{t.label}</option>))}</select></div>
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">發布日期</label><input type="date" required className="w-full p-3 rounded-xl border bg-white font-bold" value={formData.date} onChange={e=>setFormData({...formData, date:e.target.value})} /></div>
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase">下架日期</label><input type="date" className="w-full p-3 rounded-xl border bg-white font-bold" value={formData.endDate || ''} onChange={e=>setFormData({...formData, endDate:e.target.value})} /></div>
          </div>
          <div className="space-y-1.5 pt-2"><label className="text-[10px] font-black text-slate-400 uppercase">詳細內容</label><textarea placeholder="輸入內容..." rows="4" className="w-full p-4 rounded-xl border bg-white font-bold" value={formData.content} onChange={e=>setFormData({...formData, content:e.target.value})} /></div>
          <div className="flex items-center justify-between pt-2">
            <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" className="w-4 h-4 rounded text-rose-500" checked={formData.isNew} onChange={e=>setFormData({...formData, isNew:e.target.checked})} /><span className="text-sm font-bold text-slate-600">標示為 <span className="bg-rose-500 text-white text-[9px] px-1.5 py-0.5 rounded shadow-sm font-black uppercase ml-1">New</span> 新訊</span></label>
            <div className="flex gap-3"><button type="submit" className="px-8 py-3 rounded-xl font-black text-white bg-rose-500 hover:bg-rose-600 shadow-md flex items-center gap-2">{editingId ? <><Edit2 size={18}/> 更新公告</> : <><Plus size={18}/> 發布公告</>}</button></div>
          </div>
        </form>
      </div>
    </div>
...
const App = () => {
  ...
  const [announcements, setAnnouncements] = useState([
    { id: 1, type: 'policy', title: '2026年員工旅遊補助辦法及申請期限更新', date: '2026-04-15', endDate: '2026-05-15', isNew: true, content: '請各位同仁注意，2026年度的員工旅遊補助辦法已於今日更新。' },
    { id: 2, type: 'system', title: '系統維護通知：本週五晚間 10:00-12:00 暫停各項表單申請', date: '2026-04-14', endDate: '2026-04-18', isNew: false, content: '資訊部預計於本週五晚間 10:00 至 12:00 進行伺服器例行性維護。' },
  ]);
  ...
  const fetchData = async () => { 
    try { 
      ...
      const [resEmp, resRec] = await Promise.all([ 
        fetch(`${NGROK_URL}/api/employees`, fetchOptions).then(r => r.ok ? r.json() : []), 
        fetch(`${NGROK_URL}/api/records`, fetchOptions).then(r => r.ok ? r.json() : []) 
      ]); 
      setEmployees(resEmp); setRecords(resRec); 
    } catch (err) { setApiError(true); setEmployees([]); setRecords([]); } finally { setLoading(false); }
  };
  ...
  const otSerialId = useMemo(() => {
    ...
    const todays = records.filter(r => r.serialId?.startsWith(dateStr) && r.formType === '加班');
    return `${dateStr}-OT${String(todays.length + 1).padStart(3, '0')}`;
  }, [records]);

  const leaveSerialId = useMemo(() => {
    const dateStr = new Date().toISOString().split('T')[0].replace(/-/g, '');
    const todays = records.filter(r => r.serialId?.startsWith(dateStr) && r.formType === '請假');
    return `${dateStr}-LV${String(todays.length + 1).padStart(3, '0')}`;
  }, [records]);
  ...
  return (
    <div className="h-screen w-full bg-slate-50 flex text-left font-sans text-slate-900 overflow-hidden">
      ...
      <aside className="w-80 bg-white border-r border-slate-200 p-8 flex flex-col h-full shadow-sm shrink-0 z-20">
        ...
        <nav className="space-y-2 flex-grow overflow-y-auto">
          ...
          <button onClick={() => setActiveMenu('welcome')} className={`w-full flex items-center gap-4 p-4 rounded-2xl font-bold transition-all border-l-4 ${activeMenu === 'welcome' ? 'bg-sky-50 text-sky-600 border-sky-600 shadow-sm' : 'text-slate-400 hover:bg-slate-50 border-transparent'}`}><Sparkles size={20} /> 首頁總覽</button>
          <button onClick={() => setActiveMenu('announcement-list')} className={`w-full flex items-center gap-4 p-4 rounded-2xl font-bold transition-all border-l-4 ${activeMenu === 'announcement-list' ? 'bg-yellow-50 text-yellow-600 border-yellow-500 shadow-sm' : 'text-slate-400 hover:bg-slate-50 border-transparent'}`}><Bell size={20} /> 資訊公告</button>
          ...
          <button onClick={() => setActiveMenu('overtime')} className={`w-full flex items-center gap-4 p-4 rounded-2xl font-bold transition-all border-l-4 ${activeMenu === 'overtime' ? 'bg-blue-50 text-blue-600 border-blue-600 shadow-sm' : 'text-slate-400 hover:bg-slate-50 border-transparent'}`}><Clock size={20} /> 加班申請</button>
          <button onClick={() => setActiveMenu('leave-apply')} className={`w-full flex items-center gap-4 p-4 rounded-2xl font-bold transition-all border-l-4 ${activeMenu === 'leave-apply' ? 'bg-emerald-50 text-emerald-600 border-emerald-600 shadow-sm' : 'text-slate-400 hover:bg-slate-50 border-transparent'}`}><CalendarPlus size={20} /> 請假申請</button>
          ...
          {isAdmin && (
            <>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-4 mt-8 mb-2">管理功能區</p>
              ...
              <button onClick={() => setActiveMenu('personnel')} className={`w-full flex items-center gap-4 p-4 rounded-2xl font-bold transition-all border-l-4 ${activeMenu === 'personnel' ? 'bg-teal-50 text-teal-600 border-teal-600 shadow-sm' : 'text-slate-400 hover:bg-slate-50 border-transparent'}`}><Users size={20} /> 人員管理</button>
            </>
          )}
        </nav>
        <div className="mt-auto space-y-4">
          <div className="p-4 bg-slate-50 rounded-3xl border border-slate-100 flex items-center gap-3">
            <div className="px-2 min-w-[40px] h-10 bg-sky-100 rounded-2xl flex items-center justify-center font-black text-sky-600 text-[10px] shadow-inner">{userSession.dept || '部門'}</div>
            <div className="overflow-hidden"><p className="text-xs font-black truncate">{userSession.name}</p><p className="text-[10px] text-slate-400 font-mono font-bold">{userSession.empId}</p></div>
          </div>
          <button onClick={() => setUserSession(null)} className="w-full flex items-center gap-3 p-4 rounded-2xl font-bold text-rose-500 hover:bg-rose-50 transition-all active:scale-95"><LogOut size={20} /> 登出系統</button>
        </div>
      </aside>
