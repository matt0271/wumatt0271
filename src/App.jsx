    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6 font-sans">
      <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden max-w-md w-full animate-in zoom-in-95 duration-500">
        ...
        <form onSubmit={handleLogin} className="p-10 space-y-6">
          ...
          <button disabled={loading} className="w-full py-4 rounded-2xl font-black text-white bg-sky-600 shadow-xl hover:bg-sky-700 active:scale-95 flex items-center justify-center gap-3 text-white">
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 text-left text-slate-900">
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden font-sans">
        ...
        <div className="p-8 space-y-8 text-left text-slate-900">
          ...
          <div className="p-6 bg-slate-50 rounded-2xl border grid grid-cols-1 lg:grid-cols-12 gap-4 items-end text-left text-slate-900">
            ...
            <div className={`${appType === 'pre' ? 'bg-sky-500' : 'bg-rose-500'} rounded-2xl p-3 text-white flex flex-col justify-center items-center lg:col-span-2 min-h-[66px] font-black transition-colors duration-500 text-white`}><span className="text-[9px] uppercase opacity-70">時數</span><div className="flex items-baseline gap-1"><span className="text-xl text-white">{totalHours || "0"}</span><span className="text-[9px] text-white">HR</span></div></div>
          </div>
          ...
          <button disabled={totalHours <= 0 || submitting} className={`w-full py-4 rounded-2xl font-black text-white shadow-xl transition-all active:scale-[0.98] ${totalHours <= 0 || submitting ? 'bg-slate-300' : (appType === 'pre' ? 'bg-sky-500 hover:bg-sky-600' : 'bg-rose-500 hover:bg-rose-600')}`}>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 text-left text-slate-900 font-sans">
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden text-left text-slate-900">
        <div className="bg-emerald-500 px-8 py-10 text-white relative flex justify-between items-center text-white">
          <div><h1 className="text-2xl font-black text-white text-left text-white">請假申請單</h1><p className="text-sm opacity-80 text-left text-white text-white">填寫申請時段與具體理由</p></div>
          <div className="bg-white/10 backdrop-blur-md px-4 py-2 rounded-full border border-white/20 font-mono font-bold text-xs shadow-sm text-white text-white">
            <span className="opacity-70 mr-1 text-white">NO.</span>{currentSerialId}
          </div>
        </div>
        <form onSubmit={handleSubmit} className="p-8 space-y-6 text-left text-slate-900">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 text-left text-slate-900">
            <div className="space-y-1 text-left"><label className="text-[10px] font-black text-slate-400 flex items-center gap-1 uppercase">員編 <HelpCircle size={10} className="text-slate-300" /></label><input type="text" className="w-full p-3 rounded-xl border bg-white font-mono font-bold text-slate-900 outline-none focus:ring-2 focus:ring-emerald-500" value={formData.empId} onChange={e=>handleEmpIdChange(e.target.value)} /></div>
            <div className="space-y-1 text-left"><label className="text-[10px] font-black text-slate-400 flex items-center gap-1 uppercase">姓名 <HelpCircle size={10} className="text-slate-300" /></label><input type="text" className="w-full p-3 rounded-xl border bg-white font-bold text-slate-900 outline-none focus:ring-2 focus:ring-emerald-500" value={formData.name} onChange={e=>handleNameChange(e.target.value)} /></div>
            ...
            <div className="bg-emerald-500 rounded-2xl p-4 text-white flex flex-col justify-center items-center shadow-lg font-black text-white text-center"><span className="text-[10px] opacity-80 uppercase text-white">總時數</span><div className="flex items-baseline gap-1 text-white"><span className="text-2xl text-white">{totalHours || "0"}</span><span className="text-[10px] text-white">HR</span></div></div>
          </div>
          <div className="p-6 bg-slate-50 rounded-2xl border grid grid-cols-1 lg:grid-cols-2 gap-8 items-end text-left text-slate-900">
            <div className="text-left text-slate-900"><label className="text-xs font-bold text-emerald-600 flex items-center gap-2 mb-2 font-black text-left text-emerald-600">開始時間</label><div className="flex gap-2 text-slate-900 text-left text-slate-900"><input type="date" required className="flex-1 p-3 rounded-xl border" value={formData.startDate} onChange={e=>setFormData({...formData, startDate:e.target.value, endDate:e.target.value})} /><select className="p-3 w-20 rounded-xl border font-bold bg-white text-slate-900" value={formData.startHour} onChange={e=>setFormData({...formData, startHour:e.target.value})} required>{HOURS.map(h=><option key={h} value={h}>{h}</option>)}</select><select className="p-3 w-20 rounded-xl border font-bold bg-white text-slate-900" value={formData.startMin} onChange={e=>setFormData({...formData, startMin:e.target.value})} required>{MINUTES.map(m=><option key={m} value={m}>{m}</option>)}</select></div></div>
            <div className="text-left text-slate-900"><label className="text-xs font-bold text-rose-500 flex items-center gap-2 mb-2 font-black text-left text-rose-500">結束時間</label><div className="flex gap-2 text-slate-900 text-left text-slate-900"><input type="date" required className="flex-1 p-3 rounded-xl border" value={formData.endDate} onChange={e=>setFormData({...formData, endDate:e.target.value})} /><select className="p-3 w-20 rounded-xl border font-bold bg-white text-slate-900" value={formData.endHour} onChange={e=>setFormData({...formData, endHour:e.target.value})} required>{HOURS.map(h=><option key={h} value={h}>{h}</option>)}</select><select className="p-3 w-20 rounded-xl border font-bold bg-white text-slate-900" value={formData.endMin} onChange={e=>setFormData({...formData, endMin:e.target.value})} required>{MINUTES.map(m=><option key={m} value={m}>{m}</option>)}</select></div></div>
          </div>
          <div className="space-y-1 text-left text-slate-900"><label className="text-[10px] font-black text-slate-400 uppercase">請假理由</label><textarea required rows="3" className="w-full p-4 rounded-xl border bg-white font-bold text-slate-900 outline-none focus:ring-4 focus:ring-emerald-50 text-left" placeholder="請輸入詳細請假原因..." value={formData.reason} onChange={e=>setFormData({...formData, reason:e.target.value})} /></div>
...
const InquiryView = ({ records, userSession }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 text-left text-slate-900 font-sans text-left text-slate-900">
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden text-left text-slate-900">
        <div className="bg-amber-400 px-8 py-10 text-white flex justify-between items-center text-white text-white">
          <div><h1 className="text-2xl font-black text-white text-left text-white">申請單據查詢</h1><p className="text-sm opacity-90 italic text-white text-left text-white text-white text-white">查看近 30 天內所有單據狀態</p></div><ClipboardList size={40} className="opacity-30 text-white text-white" />
        </div>
        <div className="p-8 space-y-4 text-left text-slate-900 text-left text-slate-900 text-left">
          {myRecords.length > 0 ? myRecords.map(r => (
            <div key={r.id} className="bg-slate-50 p-6 rounded-2xl border flex items-center justify-between hover:border-amber-300 transition-all shadow-sm text-left text-slate-900">
              <div className="flex items-center gap-8 text-left text-slate-900 text-left">
                <div className="text-left"><p className="text-[10px] font-black text-slate-400 uppercase text-left text-slate-400">類型</p><span className={`px-2 py-1 rounded-lg text-[10px] font-black ${r.formType === '請假' ? 'bg-emerald-50 text-emerald-700' : 'bg-sky-50 text-sky-700'}`}>{r.formType}</span></div>
                <div className="text-left"><p className="text-[10px] font-black text-slate-400 uppercase text-left text-slate-400">單號</p><p className="font-mono font-bold text-amber-600 text-left text-amber-600">{r.serialId}</p></div>
                <div className="text-left"><p className="text-[10px] font-black text-slate-400 text-left text-slate-400 text-left">時數</p><p className="font-black text-left text-slate-900">{r.totalHours} HR</p></div>
              </div>
              <StatusBadge status={r.status} />
            </div>
          )) : <div className="py-24 text-center text-slate-300 italic font-bold text-slate-300">目前無單據紀錄</div>}
        </div>
      </div>
    </div>
  );
...
const ChangePasswordView = ({ userSession, setNotification, onLogout }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 text-left font-sans text-slate-900 text-left text-slate-900">
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden text-left text-slate-900 text-left">
        <div className="bg-violet-500 px-8 py-10 text-white flex justify-between items-center text-white text-white text-white">
          <div className="text-left text-white text-white text-white text-white"><h1 className="text-2xl font-black text-white text-white text-white">帳號安全設定</h1><p className="text-sm opacity-90 italic text-white text-white text-white">變更後將強制登出以確認生效</p></div><KeyRound size={40} className="opacity-30 text-white text-white" />
        </div>
        <form onSubmit={handleUpdate} className="p-10 space-y-8 max-w-lg mx-auto py-16 text-left text-slate-900 text-left">
          <div className="space-y-6 text-left text-slate-900 text-left">
          ...
          <button disabled={loading} className="w-full py-5 rounded-2xl font-black text-white bg-violet-500 hover:bg-violet-600 shadow-xl active:scale-95 flex items-center justify-center gap-3 text-white text-white text-center">
            {loading ? <Loader2 size={20} className="animate-spin text-white text-white" /> : <CheckCircle size={20} className="text-white text-white" />} 儲存變更
          </button>
        </form>
      </div>
    </div>
...
const ApprovalView = ({ records, onRefresh, setNotification }) => {
  ...
  return (
    <div className="space-y-6 pb-20 text-left font-sans text-slate-900 text-left text-slate-900 text-left">
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden text-left text-slate-900 text-left text-slate-900">
        <div className="bg-indigo-600 p-8 text-white flex justify-between items-center text-white text-left text-white text-left text-white">
          <div className="space-y-1 text-left text-white">
            <div className="flex items-center gap-2 mb-1 text-left text-white"><span className="px-2 py-0.5 bg-white/20 rounded text-[10px] font-bold uppercase tracking-wider text-white">待審核名單</span></div>
            <h1 className="text-2xl font-black text-left text-white">主管審核中心</h1>
            <p className="text-sm opacity-90 font-medium italic text-left text-white text-white text-white">審核員工提交之加班或請假申請單</p>
          </div>
          <ShieldCheck size={40} className="opacity-40 text-white text-white text-white" />
        </div>
        <div className="overflow-x-auto text-left text-slate-900 text-left">
          <table className="w-full text-left border-collapse text-sm text-left text-slate-900 text-left">
            <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b text-left text-slate-400 text-left text-slate-400">
              <tr><th className="px-8 py-4 text-left text-slate-400">選擇</th><th className="px-4 py-4 text-left text-slate-400">類型</th><th className="px-4 py-4 text-left text-slate-400">單號</th><th className="px-4 py-4 text-left text-slate-400">申請人</th><th className="px-4 py-4 text-center text-slate-400">時數</th><th className="px-4 py-4 min-w-[200px] text-left text-slate-400">事由</th><th className="px-8 py-4 text-right text-slate-400 text-slate-400">狀態</th></tr>
            </thead>
            <tbody className="divide-y divide-slate-100 text-left text-slate-900 text-left">
              {pendingRecords.length > 0 ? pendingRecords.map(r => (
                <tr key={r.id} onClick={() => setSelectedId(r.id)} className={`transition-all cursor-pointer text-left ${selectedId === r.id ? 'bg-indigo-50 ring-2 ring-inset ring-indigo-200 text-left' : 'hover:bg-slate-50 text-left text-slate-900'}`}>
                  <td className="px-8 py-5 text-left text-slate-900"><div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center text-left ${selectedId === r.id ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-slate-300'}`}>{selectedId === r.id && <div className="w-2 h-2 rounded-full bg-white text-left text-white" />}</div></td>
                  <td className="px-4 py-5 text-left text-slate-900"><span className={`px-2 py-1 rounded-lg text-[10px] font-black ${r.formType === '請假' ? 'bg-emerald-50 text-emerald-700' : 'bg-sky-50 text-sky-700'}`}>{r.formType}</span></td>
                  <td className="px-4 py-5 font-mono font-bold text-left text-slate-900">{r.serialId}</td>
                  <td className="px-4 py-5 font-black text-slate-800 text-left">{r.name}<div className="text-[10px] text-slate-400 font-bold text-left text-slate-400">{r.empId}</div></td>
                  <td className="px-4 py-5 text-center font-bold text-left text-slate-900">{r.totalHours}</td>
                  <td className="px-4 py-5 text-left text-slate-900 text-xs text-left"><p className="line-clamp-2 text-left text-slate-900" title={r.reason}>{r.reason}</p></td>
                  <td className="px-8 py-5 text-right text-slate-900"><StatusBadge status={r.status} /></td>
                </tr>
              )) : (<tr><td colSpan="7" className="px-8 py-24 text-center text-slate-300 italic font-bold text-slate-300">目前無待簽核申請單</td></tr>)}
            </tbody>
          </table>
        </div>
      </div>
      {selectedId && (
        <div className="bg-white rounded-3xl shadow-xl border border-indigo-200 p-8 flex flex-col md:flex-row gap-8 text-left text-slate-900 text-left">
          <div className="flex-1 space-y-4 text-left text-slate-900 text-left">
            <div className="flex items-center gap-2 text-indigo-600 font-black text-sm text-left text-indigo-600"><MessageSquare size={18} className="text-indigo-600 text-left" /> 簽核意見 <span className="text-rose-400 font-bold text-[10px] ml-1 uppercase tracking-widest text-left text-rose-400">* 駁回為必填</span></div>
            <textarea placeholder="填寫具體簽核意見或指示..." className="w-full p-5 rounded-2xl border bg-slate-50 outline-none text-sm font-bold text-slate-900 text-left text-slate-900" value={opinion} onChange={(e) => setOpinion(e.target.value)} />
          </div>
          <div className="w-full md:w-72 flex flex-col justify-end gap-3 text-left text-slate-900 text-left">
            <p className="text-[10px] font-black text-slate-400 uppercase px-1 text-left text-slate-400">選取單據：<span className="text-indigo-600 font-bold text-left text-indigo-600">{selectedRecord?.serialId}</span></p>
            <div className="grid grid-cols-2 gap-3 text-white text-left text-white text-left">
              <button disabled={updating} onClick={() => handleUpdate('rejected')} className="flex flex-col items-center justify-center gap-2 py-4 bg-rose-50 text-rose-600 rounded-2xl border border-rose-100 hover:bg-rose-600 active:scale-95 text-[11px] font-black uppercase text-center text-rose-600 hover:text-white"><XCircle size={24}/><span className="text-center">駁回</span></button>
              <button disabled={updating} onClick={() => handleUpdate('approved')} className="flex flex-col items-center justify-center gap-2 py-4 bg-emerald-50 text-emerald-600 rounded-2xl border border-emerald-100 hover:bg-emerald-600 active:scale-95 text-[11px] font-black uppercase text-center text-emerald-600 hover:text-white">{updating ? <Loader2 size={24} className="animate-spin text-center" /> : <CheckCircle2 size={24} className="text-center" />}<span className="text-center">核准</span></button>
            </div>
          </div>
        </div>
      )}
...
const PersonnelManagement = ({ employees, onRefresh, setNotification }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 text-left font-sans text-slate-900 text-left text-slate-900 text-left text-slate-900">
      {pwdTarget && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm text-left text-slate-900 text-left text-slate-900">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center text-slate-900 text-left text-slate-900">
            <RotateCcw size={48} className="text-slate-500 mx-auto mb-4 text-slate-500 text-center" />
            <h3 className="text-xl font-black mb-1 text-slate-900 text-center text-slate-900">重設密碼？</h3>
            <p className="text-xs text-slate-400 mb-8 font-bold text-center text-slate-400 text-center">為 {pwdTarget.name} 還原為員編密碼</p>
            <div className="flex gap-3 text-left text-slate-900 text-left">
              <button onClick={()=>setPwdTarget(null)} className="flex-1 py-3 font-bold bg-slate-100 rounded-xl text-slate-900 text-center text-slate-900">取消</button>
              <button onClick={() => { fetch(`${NGROK_URL}/api/employees/${pwdTarget.id}`, { method: 'PUT', headers: fetchOptions.headers, body: JSON.stringify({ ...pwdTarget, password: pwdTarget.empId }) }).then(onRefresh); setPwdTarget(null); }} className="flex-1 py-3 font-black text-white bg-slate-600 rounded-xl text-white text-center text-white">確認</button>
            </div>
          </div>
        </div>
      )}
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden text-left text-slate-900 text-left">
        <div className="bg-slate-600 p-8 text-white flex justify-between items-center text-white text-left text-white text-left text-white">
          <div className="text-left text-white text-left text-white"><h1 className="text-2xl font-black text-white text-left text-white">人員管理中心</h1><p className="text-sm opacity-90 italic text-white text-left text-white text-white">維護同仁資料與 Excel 工具</p></div><Users size={40} className="opacity-40 text-white text-left text-white text-left" />
        </div>
        <div className="px-8 pt-6 flex gap-3 text-left text-emerald-600 text-left text-emerald-600">
          <button onClick={handleExport} className="flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl text-xs font-bold border border-emerald-100 transition-colors text-emerald-600 text-left text-emerald-600"><FileSpreadsheet size={16}/> 匯出 Excel</button>
          <button onClick={() => fileInputRef.current.click()} className="flex items-center gap-2 px-4 py-2 bg-sky-50 text-sky-600 rounded-xl text-xs font-bold border border-sky-100 transition-colors text-sky-600 text-left text-sky-600"><Upload size={16}/> 匯入 Excel</button>
          <input type="file" ref={fileInputRef} onChange={handleImport} accept=".xlsx, .xls" className="hidden" />
        </div>
        <form onSubmit={e=>{e.preventDefault(); const url=editingId?`${NGROK_URL}/api/employees/${editingId}`:`${NGROK_URL}/api/employees`; fetch(url,{method:editingId?'PUT':'POST',headers:fetchOptions.headers,body:JSON.stringify(formData)}).then(()=>{onRefresh(); setEditingId(null); setFormData({name:'',empId:'',jobTitle:'',dept:''});});}} className="p-8 space-y-6 text-left text-slate-900 text-left">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-left text-slate-900 text-left">
            <input type="text" placeholder="員編" required className="p-3 rounded-xl border bg-slate-50 outline-none text-left text-slate-900 text-left" value={formData.empId} onChange={e=>setFormData({...formData, empId:e.target.value})} />
            <input type="text" placeholder="姓名" required className="p-3 rounded-xl border bg-slate-50 outline-none text-left text-slate-900 text-left" value={formData.name} onChange={e=>setFormData({...formData, name:e.target.value})} />
            <input type="text" placeholder="職稱" required className="p-3 rounded-xl border bg-slate-50 outline-none text-left text-slate-900 text-left" value={formData.jobTitle} onChange={e=>setFormData({...formData, jobTitle:e.target.value})} />
            <input type="text" placeholder="單位" required className="p-3 rounded-xl border bg-slate-50 outline-none text-left text-slate-900 text-left" value={formData.dept} onChange={e=>setFormData({...formData, dept:e.target.value})} />
          </div>
          <button className="w-full py-4 bg-slate-600 text-white rounded-2xl font-black text-center text-white text-white text-white"> {editingId ? '更新資料' : '新增人員'} </button>
        </form>
        <div className="overflow-x-auto border-t text-left text-slate-900 text-left">
          <table className="w-full text-left border-collapse text-sm text-left text-slate-900 text-left">
            <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b text-left text-slate-400 text-left text-slate-400 text-left">
              <tr><th className="px-8 py-4 text-left text-slate-400">員編</th><th className="px-4 py-4 text-left text-slate-900">姓名</th><th className="px-4 py-4 text-slate-900 text-left text-slate-900">職稱 / 單位</th><th className="px-4 py-4 text-slate-900 text-left text-slate-900">登入密碼</th><th className="px-8 py-4 text-right text-slate-900 text-right text-slate-900 text-right">操作</th></tr>
            </thead>
            <tbody className="divide-y divide-slate-100 text-left text-slate-900 text-left text-slate-900">
              {employees.map(emp => (
                <tr key={emp.id} className="hover:bg-slate-50 text-left text-slate-900 text-left text-slate-900">
                  <td className="px-8 py-5 font-mono font-bold text-slate-600 text-left text-slate-600 text-left text-slate-600">{emp.empId}</td>
                  <td className="px-4 py-5 font-black text-slate-800 text-left text-slate-800 text-left text-slate-800">{emp.name}</td>
                  <td className="px-4 py-5 text-left text-slate-900 text-left text-slate-900 text-left text-slate-900"><div className="font-bold text-slate-900 text-left text-slate-900">{emp.jobTitle}</div><div className="text-[10px] text-slate-400 font-bold text-left text-slate-400">{emp.dept}</div></td>
                  <td className="px-4 py-5 text-left text-slate-900 text-left text-slate-900 text-left text-slate-900"><div className="flex items-center gap-3 text-left text-slate-900 text-left text-slate-900">{(emp.password && emp.password !== emp.empId) && (<span className="px-2 py-1 rounded-lg text-[10px] font-mono font-bold bg-emerald-100 text-emerald-700 text-left text-emerald-700">已自訂</span>)}<button onClick={()=>setPwdTarget(emp)} className="text-[10px] font-black text-slate-500 hover:text-slate-800 flex items-center gap-1 text-left text-slate-500 text-left text-slate-500 text-left"><RotateCcw size={12}/>還原</button></div></td>
                  <td className="px-8 py-5 text-right flex justify-end gap-2 text-slate-900 text-right text-slate-900 text-right text-slate-900 text-right">
                    <button onClick={()=>{setEditingId(emp.id);setFormData(emp);window.scrollTo({top:0,behavior:'smooth'});}} className="p-2 text-slate-300 hover:text-slate-600 text-left text-slate-300 text-left text-slate-300"><Edit2 size={16} /></button>
                    <button onClick={() => { if(window.confirm("確定刪除？")) fetch(`${NGROK_URL}/api/employees/${emp.id}`, { method: 'DELETE', headers: fetchOptions.headers }).then(onRefresh); }} className="p-2 text-slate-300 hover:text-rose-600 text-left text-slate-300 text-left text-slate-300"><Trash2 size={16} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
...
const App = () => {
  ...
  const fetchData = async () => { try { const [resEmp, resRec] = await Promise.all([ fetch(`${NGROK_URL}/api/employees`, fetchOptions).then(r => r.json()), fetch(`${NGROK_URL}/api/records`, fetchOptions).then(r => r.json()) ]); setEmployees(Array.isArray(resEmp) ? resEmp : []); setRecords(Array.isArray(resRec) ? resRec : []); setLoading(false); } catch (err) { setLoading(false); } };
  useEffect(() => { fetchData(); }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex text-left font-sans text-slate-900 overflow-hidden text-left text-slate-900 text-left">
      ...
      <aside className="w-80 bg-white border-r border-slate-200 p-8 flex flex-col sticky top-0 h-screen shadow-sm shrink-0 text-left text-slate-900 text-left text-slate-900">
        <div className="flex items-center gap-4 mb-10 text-sky-500 text-left text-sky-500 text-left text-sky-500"><div className="p-3 bg-sky-500 rounded-2xl shadow-lg text-white text-left text-white text-left text-white"><LayoutDashboard size={24} /></div><h2 className="font-black text-xl tracking-tight text-left text-sky-600 text-left">員工服務平台</h2></div>
        ...
        <div className="mt-auto space-y-4">
          ...
          <button onClick={() => setUserSession(null)} className="w-full flex items-center gap-3 p-4 rounded-2xl font-bold text-rose-500 hover:bg-rose-50 transition-all border border-transparent hover:border-rose-100 active:scale-95 text-left text-rose-500 text-left text-rose-500">
            <LogOut size={20} /> 登出系統
          </button>
        </div>
      </aside>
      <main className="flex-grow p-10 overflow-y-auto bg-slate-50 text-left text-slate-900 text-left text-slate-900 text-left text-slate-900">
