    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-100 p-6 font-sans">
      <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden max-w-md w-full animate-in zoom-in-95 duration-500 text-left">
        <div className="bg-sky-600 p-12 text-white text-center relative overflow-hidden">
          ...
          <h1 className="text-3xl font-black tracking-tight relative z-10 text-center">身分驗證</h1>
          <p className="text-sky-100 mt-2 opacity-80 text-sm relative z-10 font-bold text-center">員工服務平台 v2.8</p>
        </div>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="relative group text-left">
                ...
                <input 
                  type="text" required 
                  className="w-full pl-12 pr-4 py-4 rounded-2xl border border-slate-200 bg-slate-50 text-base focus:ring-4 focus:ring-sky-500/10 focus:border-sky-600 outline-none transition-all placeholder:text-slate-300 font-bold text-slate-900" 
                  placeholder="輸入姓名或員編"
                  value={identifier}
            ...
            <div className="space-y-2">
              ...
              <div className="relative group text-left">
                ...
                <input 
                  ...
                  className="w-full pl-12 pr-12 py-4 rounded-2xl border border-slate-200 bg-slate-50 text-base focus:ring-4 focus:ring-sky-500/10 focus:border-sky-600 outline-none transition-all placeholder:text-slate-300 font-bold text-slate-900" 
                  placeholder="輸入密碼"
                  value={password}
    ...
    <div className="space-y-8 animate-in fade-in duration-500 relative text-left">
          ...
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleWithdrawAction} disabled={withdrawing} className="flex-1 py-3 font-black text-white bg-rose-500 rounded-xl flex items-center justify-center gap-2">
                {withdrawing ? <Loader2 size={18} className="animate-spin text-white" /> : <Undo2 size={18} />} 確認
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden text-left font-sans text-slate-900">
        <div className="bg-sky-500 px-8 py-10 text-white relative">
          <div className="absolute top-6 right-8 flex items-center gap-2 bg-white/10 backdrop-blur-md px-4 py-2 rounded-full border border-white/20 font-bold text-[10px] font-mono">{currentSerialId}</div>
          <h1 className="text-2xl font-black">加班申請單</h1>
          <div className="mt-6 flex bg-white/20 p-1 rounded-xl w-fit">
            <button onClick={() => setAppType('pre')} className={`px-6 py-2 rounded-lg text-xs font-bold transition-all ${appType === 'pre' ? 'bg-white text-sky-600 shadow' : 'text-white/70'}`}>事前申請</button>
            <button onClick={() => setAppType('post')} className={`px-6 py-2 rounded-lg text-xs font-bold transition-all ${appType === 'post' ? 'bg-white text-sky-600 shadow' : 'text-white/70'}`}>事後補報</button>
          </div>
        </div>
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">員編</label><input type="text" readOnly className="w-full p-3 rounded-xl border border-slate-100 bg-slate-50 text-sm font-mono font-bold text-slate-400" value={formData.empId} /></div>
            <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">姓名</label><input type="text" readOnly className="w-full p-3 rounded-xl border border-slate-100 bg-slate-50 text-sm font-bold text-slate-400" value={formData.name} /></div>
            <div className="space-y-1 text-left"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-slate-400">類別</label><select className="w-full p-3 rounded-xl border border-slate-200 bg-white text-sm font-bold text-slate-900" value={formData.category} onChange={e=>setFormData({...formData, category:e.target.value})}>{OT_CATEGORIES.map(c=><option key={c.id} value={c.id}>{c.label}</option>)}</select></div>
            <div className="space-y-1 text-left"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-slate-400">補償方式</label>
          ...
          <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 grid grid-cols-1 lg:grid-cols-12 gap-4 items-end font-sans text-left">
            <div className="space-y-2 lg:col-span-5"><label className="text-xs font-bold text-sky-600 flex items-center gap-2"><Plus size={14} />開始時間</label>
              <div className="flex gap-2">
                <input type="date" required className="flex-1 min-w-0 p-3 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-sky-500 text-slate-900" value={formData.startDate} onChange={e=>setFormData({...formData, startDate:e.target.value, endDate:e.target.value})} />
            ...
            <div className="space-y-2 lg:col-span-5"><label className="text-xs font-bold text-rose-400 flex items-center gap-2"><ArrowRight size={14} />結束時間</label>
              <div className="flex gap-2">
            ...
            <div className="bg-sky-500 rounded-2xl p-3 text-white flex flex-col justify-center items-center shadow-lg lg:col-span-2 min-h-[66px]">
              <span className="text-[9px] font-black uppercase opacity-70">預計時數</span>
              <div className="flex items-baseline gap-1 font-sans font-black"><span className="text-xl">{totalHours || "0"}</span><span className="text-[9px] opacity-60">HR</span></div>
            </div>
          </div>
          <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">加班詳細事由</label><textarea required rows="3" placeholder="請填寫工作內容原因..." className="w-full p-4 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:bg-white transition-all text-slate-900 font-bold" value={formData.reason} onChange={e=>setFormData({...formData, reason:e.target.value})} /></div>
          <button disabled={totalHours <= 0 || submitting} className={`w-full py-4 rounded-2xl font-black text-white shadow-xl flex items-center justify-center gap-3 transition-all active:scale-[0.98] ${totalHours <= 0 || submitting ? 'bg-slate-300':'bg-sky-500 hover:bg-sky-600 shadow-sky-100'}`}>
            {submitting ? <Loader2 size={18} className="animate-spin text-white" /> : <ClipboardCheck size={18} className="text-white" />} {submitting ? '處理中...' : '送出加班申請'}
          </button>
        </form>
      </div>
    </div>
    ...
    <div className="space-y-8 animate-in fade-in duration-500 text-left text-slate-900">
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden text-left font-sans text-slate-900">
        <div className="bg-emerald-500 px-8 py-10 text-white relative">
          ...
          <p className="mt-2 text-emerald-100 text-sm opacity-80">請填寫請假時段與具體理由</p>
        </div>
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">申請人</label><div className="p-3 bg-slate-50 border rounded-xl text-sm font-bold text-slate-400">{userSession.empId} - {userSession.name}</div></div>
            ...
            <div className="bg-emerald-500 rounded-2xl p-3 text-white flex flex-col justify-center items-center shadow-lg font-black">
              <span className="text-[9px] uppercase opacity-70">申請總時數</span>
              <div className="flex items-baseline gap-1"><span className="text-xl">{totalHours || "0"}</span><span className="text-[9px] opacity-60">HR</span></div>
            </div>
          </div>
          <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 grid grid-cols-1 lg:grid-cols-2 gap-6 items-end font-sans text-slate-900">
            <div className="space-y-2"><label className="text-xs font-bold text-emerald-600 flex items-center gap-2"><Plus size={14} />開始時間</label>
              <div className="flex gap-2">
                <input type="date" required className="flex-1 p-3 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-emerald-500 text-slate-900" value={formData.startDate} onChange={e=>setFormData({...formData, startDate:e.target.value, endDate:e.target.value})} />
            ...
            <div className="space-y-2"><label className="text-xs font-bold text-rose-400 flex items-center gap-2"><ArrowRight size={14} />結束時間</label>
              <div className="flex gap-2">
                <input type="date" required className="flex-1 p-3 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-emerald-500 text-slate-900" value={formData.endDate} onChange={e=>setFormData({...formData, endDate:e.target.value})} />
          ...
          <div className="space-y-2 text-left text-slate-900"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">請假理由</label><textarea required rows="3" placeholder="詳細說明請假具體原因..." className="w-full p-4 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:bg-white transition-all text-slate-900 font-bold" value={formData.reason} onChange={e=>setFormData({...formData, reason:e.target.value})} /></div>
          <button disabled={totalHours <= 0 || submitting} className={`w-full py-4 rounded-2xl font-black text-white shadow-xl flex items-center justify-center gap-3 transition-all active:scale-[0.98] ${totalHours <= 0 || submitting ? 'bg-slate-300' : 'bg-emerald-500 hover:bg-emerald-600 shadow-emerald-100'}`}>
            {submitting ? <Loader2 size={18} className="animate-spin text-white" /> : <CalendarPlus size={18} />} {submitting ? '提交中...' : '送出請假申請'}
          </button>
        </form>
      </div>
    </div>
...
const InquiryView = ({ records, userSession }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 text-left text-slate-900">
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden text-left font-sans text-slate-900">
        <div className="bg-amber-500 px-8 py-8 text-white flex justify-between items-center text-left text-white">
          <div className="text-left text-white"><h1 className="text-2xl font-black text-left">單據查詢</h1><p className="text-sm opacity-80 italic font-bold text-left">追蹤最近 30 天內的加班與請假紀錄</p></div>
        ...
        <div className="p-8">
          {myRecords.length > 0 ? (
              ...
              {myRecords.map(r => (
                <div key={r.id} className="bg-slate-50 p-6 rounded-2xl border border-slate-200 flex flex-wrap items-center justify-between gap-6 hover:border-amber-300 transition-all shadow-sm">
                  <div className="flex items-center gap-8">
                    <div className="space-y-1 text-left text-slate-900"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">類型</p><span className={`px-2 py-1 rounded-lg text-[10px] font-black ${r.formType === '請假' ? 'bg-emerald-100 text-emerald-700' : 'bg-sky-100 text-sky-700'}`}>{r.formType}</span></div>
                    <div className="space-y-1 text-left text-slate-900"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">單號</p><p className="font-mono font-black text-amber-600 text-sm">{r.serialId}</p></div>
                    <div className="space-y-1 text-left text-slate-900"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">時數</p><p className="font-black text-slate-700 text-sm">{r.totalHours} HR</p></div>
                    <div className="space-y-1 hidden md:block text-left text-slate-900 text-left text-slate-900"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">時間</p><p className="text-xs font-bold text-slate-500">{r.startDate} ~ {r.endDate}</p></div>
                  </div>
            ...
            <div className="py-20 text-center text-slate-400 italic font-bold">目前尚無單據申請紀錄</div>
          )}
        </div>
      </div>
    </div>
...
const ApprovalView = ({ records, onRefresh, setNotification }) => {
  ...
  const updateStatus = async (status) => {
    ...
    if (status === 'rejected' && !opinion.trim()) return setNotification({ type: 'error', text: '駁回必填理由' });
    setUpdating(true);
    try {
  ...
  return (
    <div className="space-y-6 pb-20 text-left font-sans text-slate-900">
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden text-left text-slate-900">
        <div className="bg-indigo-500 p-8 text-white flex justify-between items-center text-left text-white">
          <div className="text-left text-white"><h1 className="text-2xl font-black text-left text-white">主管簽核</h1><p className="text-sm opacity-80 italic font-bold text-left text-white">審核待處理之員工申請 (加班/請假)</p></div><ShieldCheck size={40} className="opacity-40" />
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b text-slate-400">
              <tr className="text-left border-b"><th className="px-8 py-4 text-left">選擇</th><th className="px-4 py-4 text-left">類型</th><th className="px-4 py-4 text-left">單號</th><th className="px-4 py-4 text-left">申請人</th><th className="px-4 py-4 text-left">期間</th><th className="px-4 py-4 text-left min-w-[200px]">事由</th><th className="px-8 py-4 text-right">狀態</th></tr>
            </thead>
            <tbody className="divide-y divide-slate-100 text-sm">
              {pendingRecords.length > 0 ? pendingRecords.map(r => (
                <tr key={r.id} onClick={() => setSelectedId(r.id)} className={`transition-all cursor-pointer text-left text-slate-900 ${selectedId === r.id ? 'bg-indigo-50 ring-2 ring-inset ring-indigo-200' : 'hover:bg-slate-50'}`}>
                  <td className="px-8 py-5 text-left text-slate-900"><div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${selectedId === r.id ? 'border-indigo-500 bg-indigo-500' : 'border-slate-300'}`}>{selectedId === r.id && <div className="w-2 h-2 rounded-full bg-white text-left" />}</div></td>
                  <td className="px-4 py-5"><span className={`px-2 py-1 rounded-lg text-[10px] font-black ${r.formType === '請假' ? 'bg-emerald-100 text-emerald-700' : 'bg-sky-100 text-sky-700'}`}>{r.formType}</span></td>
                  <td className="px-4 py-5 font-mono text-indigo-600 font-bold">{r.serialId}</td>
                  <td className="px-4 py-5 font-black text-slate-800">{r.name}<div className="text-[10px] text-slate-400 font-bold tracking-tight">{r.empId}</div></td>
                  <td className="px-4 py-5 text-xs font-bold text-slate-500">{r.startDate}<br/>{r.endDate}</td>
                  <td className="px-4 py-5"><p className="text-xs text-slate-500 line-clamp-2 leading-relaxed font-bold" title={r.reason}>{r.reason}</p></td>
                  <td className="px-8 py-5 text-right"><StatusBadge status={r.status} /></td>
                </tr>
              )) : (<tr><td colSpan="7" className="px-8 py-10 text-center text-slate-400 italic font-bold">尚無待處理申請</td></tr>)}
            </tbody>
          </table>
        </div>
      </div>
      <div className={`bg-white rounded-3xl shadow-xl border p-8 transition-all duration-500 ${selectedId ? 'border-indigo-200 opacity-100' : 'border-slate-100 opacity-50 grayscale pointer-events-none translate-y-4'}`}>
        <div className="flex flex-col md:flex-row gap-8">
          <div className="flex-1 space-y-4 text-left">
            <div className="flex items-center gap-2 text-indigo-600 font-black text-sm"><MessageSquare size={18} /> 簽核意見 <span className="text-rose-400 font-bold text-[10px] ml-1 uppercase tracking-widest">* 駁回為必填</span></div>
            <textarea placeholder="填寫核准或駁回原因..." className="w-full p-4 rounded-2xl border border-slate-200 bg-slate-50 outline-none text-sm h-24 focus:bg-white focus:ring-2 focus:ring-indigo-500 transition-all font-bold" value={opinion} onChange={(e) => setOpinion(e.target.value)} />
          </div>
          <div className="w-full md:w-72 flex flex-col justify-end gap-3 text-left">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">目前選取：<span className="text-indigo-600 font-bold">{selectedRecord?.serialId || '未選取'}</span></p>
            <div className="grid grid-cols-2 gap-3">
              <button disabled={!selectedId || updating} onClick={() => updateStatus('rejected')} className="flex flex-col items-center justify-center gap-2 py-4 bg-rose-50 text-rose-600 rounded-2xl border border-rose-100 hover:bg-rose-600 hover:text-white transition-all group font-black active:scale-95 text-left"><XCircle size={24}/><span className="text-[11px] uppercase tracking-widest">駁回申請</span></button>
              <button disabled={!selectedId || updating} onClick={() => updateStatus('approved')} className="flex flex-col items-center justify-center gap-2 py-4 bg-emerald-50 text-emerald-600 rounded-2xl border border-emerald-100 hover:bg-emerald-600 hover:text-white transition-all group font-black active:scale-95 text-left">{updating ? <Loader2 size={24} className="animate-spin text-left" /> : <CheckCircle2 size={24} />}<span className="text-[11px] uppercase tracking-widest">核准單據</span></button>
            </div>
          </div>
        </div>
      </div>
...
const PersonnelManagement = ({ employees, onRefresh, setNotification }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) { setNotification({ type: 'success', text: '人員資料更新完成' }); setFormData({ name:'', empId:'', jobTitle:'', dept:'' }); setEditingId(null); onRefresh(); }
  ...
  const handleResetPassword = async (emp) => {
    ...
    try {
      ...
      if (res.ok) { setNotification({ type: 'success', text: '密碼已成功重設' }); onRefresh(); }
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden text-left animate-in fade-in duration-500 font-sans text-slate-900">
      {pwdTarget && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
            <KeyRound size={48} className="text-slate-500 mx-auto mb-4" />
            <h3 className="text-xl font-black mb-1">重設登入密碼</h3>
            <p className="text-xs text-slate-400 mb-8 font-bold">將為 {pwdTarget.name} 還原預設密碼</p>
            <div className="flex gap-3">
              <button onClick={()=>setPwdTarget(null)} className="flex-1 py-3 font-bold bg-slate-100 rounded-xl">取消</button>
              <button onClick={() => { handleResetPassword(pwdTarget); setPwdTarget(null); }} className="flex-1 py-3 font-black text-white bg-slate-600 rounded-xl flex items-center justify-center gap-2">
                確認重設
              </button>
            </div>
          </div>
      ...
      <div className="bg-slate-600 p-8 text-white flex justify-between items-center">
        <div className="text-left text-white"><h1 className="text-2xl font-black text-left text-white">人員管理</h1><p className="text-sm opacity-80 italic font-bold text-left text-white">維護企業內部人員資料庫</p></div><Users size={40} className="opacity-40" />
      </div>
      <form onSubmit={handleSubmit} className="p-8 space-y-6 text-left">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="space-y-1 text-left"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">員編</label><input type="text" required className="w-full p-3 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:ring-2 focus:ring-slate-500 outline-none font-mono font-bold text-slate-900" value={formData.empId} onChange={e=>setFormData({...formData, empId:e.target.value})} /></div>
          <div className="space-y-1 text-left"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">姓名</label><input type="text" required className="w-full p-3 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:ring-2 focus:ring-slate-500 outline-none font-bold text-slate-900" value={formData.name} onChange={e=>setFormData({...formData, name:e.target.value})} /></div>
          <div className="space-y-1 text-left"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">職稱</label><input type="text" required className="w-full p-3 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:ring-2 focus:ring-slate-500 outline-none font-bold text-slate-900" value={formData.jobTitle} onChange={e=>setFormData({...formData, jobTitle:e.target.value})} /></div>
          <div className="space-y-1 text-left"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">單位</label><input type="text" required className="w-full p-3 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:ring-2 focus:ring-slate-500 outline-none font-bold text-slate-900" value={formData.dept} onChange={e=>setFormData({...formData, dept:e.target.value})} /></div>
        </div>
        <div className="flex gap-3">
          <button type="button" onClick={()=>{setFormData({name:'',empId:'',jobTitle:'',dept:''});setEditingId(null);}} className="px-6 py-4 rounded-2xl font-bold text-slate-500 bg-slate-100 border flex items-center justify-center gap-2 hover:bg-slate-200 transition-all uppercase tracking-widest text-xs active:scale-95"><RotateCcw size={18} className="text-slate-400" /> 重設</button>
          <button disabled={loading} className={`flex-1 py-4 rounded-2xl font-black text-white shadow-xl flex items-center justify-center gap-2 transition-all uppercase tracking-widest text-xs active:scale-95 ${editingId ? 'bg-orange-500 shadow-orange-100':'bg-slate-600 shadow-slate-100'}`}>{loading ? <Loader2 size={18} className="animate-spin text-white" /> : editingId ? <Edit2 size={18} /> : <UserPlus size={18} />} {editingId ? '更新資料' : '新增人員'}</button>
        </div>
      </form>
      <div className="overflow-x-auto border-t">
        <table className="w-full text-left border-collapse">
          <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b">
            <tr><th className="px-8 py-4 text-left">員編</th><th className="px-4 py-4 text-left">姓名</th><th className="px-4 py-4 text-left text-slate-900">職稱 / 單位</th><th className="px-4 py-4 text-left text-slate-900">登入密碼</th><th className="px-8 py-4 text-right text-slate-900">操作</th></tr>
          </thead>
          <tbody className="divide-y divide-slate-100 text-sm text-left">
            {employees.map(emp => (
              <tr key={emp.id} className="hover:bg-slate-50 transition-all text-left text-slate-900">
                ...
                <td className="px-4 py-5 text-left text-slate-900">
                  ...
                  <div className="text-[10px] text-slate-400 text-left font-bold">{emp.dept}</div>
                </td>
                <td className="px-4 py-5 text-left text-slate-900">
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex text-left font-sans text-slate-900 overflow-hidden text-left">
      {notification && (
      ...
      <aside className="w-80 bg-white border-r border-slate-200 p-8 flex flex-col sticky top-0 h-screen shadow-sm shrink-0">
        ...
        <nav className="space-y-2 flex-grow">
          ...
          <button onClick={() => setActiveMenu('overtime')} className={`w-full flex items-center gap-4 p-4 rounded-2xl font-bold transition-all border-l-4 ${activeMenu === 'overtime' ? 'bg-sky-50 text-sky-600 border-sky-600 shadow-sm' : 'text-slate-400 hover:bg-slate-50 border-transparent'}`}><Clock size={20} /> 加班申請</button>
        ...
        <div className="mt-auto space-y-4">
          <div className="p-4 bg-slate-50 rounded-3xl border border-slate-100 flex items-center gap-3">
            <div className="px-2 min-w-[40px] h-10 bg-sky-100 rounded-2xl flex items-center justify-center font-black text-sky-600 shadow-inner text-[10px] whitespace-nowrap overflow-hidden">{userSession.dept || '部門'}</div>
          ...
          <button onClick={() => setUserSession(null)} className="w-full flex items-center gap-3 p-4 rounded-2xl font-bold text-rose-500 hover:bg-rose-50 transition-all border border-transparent hover:border-rose-100 uppercase tracking-widest text-xs active:scale-95">
            <LogOut size={20} /> 登出系統
          </button>
        </div>
      </aside>
