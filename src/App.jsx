...
import { 
  ...
  Paperclip, UploadCloud, Activity, GitBranch, MapPin, FastForward
} from 'lucide-react';
...
const WorkflowModal = ({ record, employees, onClose }) => {
  ...
  const workflowSteps = useMemo(() => {
    ...
    // 1. 提交階段
    steps.push({ id: 'submitted', label: '提交申請', desc: `${record.name} 已送出單據`, icon: FileText, color: 'text-sky-500', isPast: true });
    ...
    if (record.formType === '請假') {
      ...
      steps.push({ 
        id: 'pending_substitute', 
        label: '職務代理確認', 
        desc: `代理人：${record.substitute}`, 
        icon: UserCheck, 
        isPast, 
        isCurrent, 
        isSkipped: false 
    ...
    const isManagerPast = !['pending_substitute', 'pending_manager', 'pending'].includes(record.status);
    const isManagerCurrent = record.status === 'pending_manager' || record.status === 'pending';
    steps.push({ 
      id: 'pending_manager', 
      label: '單位經副理簽核', 
      desc: skipManager ? '申請人職級跳過此階' : '審核 3 日(含)以下假單', 
      icon: ShieldCheck, 
      isPast: isManagerPast || skipManager, 
      isCurrent: !skipManager && isManagerCurrent,
      isSkipped: skipManager
    });

    // 4. 協理階段
    const skipDirector = isDirector || isGM;
    const isDirectorPast = !['pending_substitute', 'pending_manager', 'pending', 'pending_director'].includes(record.status);
    const isDirectorCurrent = record.status === 'pending_director';
    steps.push({ 
      id: 'pending_director', 
      label: '單位協理簽核', 
      desc: skipDirector ? '申請人職級跳過此階' : '審核 5 日(含)以下假單', 
      icon: ShieldCheck, 
      isPast: isDirectorPast || skipDirector, 
      isCurrent: !skipDirector && isDirectorCurrent,
      isSkipped: skipDirector
    });

    // 5. 總經理階段
    const isGMPast = record.status === 'pending_assignment' || record.status === 'approved';
    const isGMCurrent = record.status === 'pending_gm';
    steps.push({ 
      id: 'pending_gm', 
      label: '總經理核定', 
      desc: '審核 5 日以上假單或主管假單', 
      icon: ShieldCheck, 
      isPast: isGMPast, 
      isCurrent: isGMCurrent,
      isSkipped: isGM && record.formType === '加班' // 加班單若由總經理申請通常直跳交辦
    });
    ...
    if (record.status === 'rejected') {
      steps.push({ id: 'rejected', label: '簽核駁回', desc: '單據已被主管退回', icon: XCircle, color: 'text-rose-500', isCurrent: true });
    } else if (record.status === 'canceled') {
      steps.push({ id: 'canceled', label: '申請撤銷', desc: '單據已由申請人撤銷', icon: RotateCcw, color: 'text-slate-500', isCurrent: true });
    } else {
            ...
            {workflowSteps.map((step, idx) => {
              ...
              return (
                <div key={idx} className="relative group">
                  {!isLast && (
                    <div className={`absolute left-[19px] top-10 bottom-0 w-0.5 transition-colors duration-500 ${isPast ? 'bg-indigo-500' : 'bg-slate-200 border-dashed'}`}></div>
                  )}
                  
                  <div className="flex gap-6 pb-10">
                    <div className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center transition-all duration-500 ${
                      isSkipped ? 'bg-slate-100 text-slate-300 border-2 border-slate-50' :
                      isCurrent ? 'bg-indigo-600 text-white shadow-[0_0_15px_rgba(79,70,229,0.4)] scale-110' : 
                      isPast ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-400'
                    }`}>
                      {isPast && !isCurrent && !isSkipped ? <Check size={20} strokeWidth={3} /> : <Icon size={20} />}
                      {isCurrent && <div className="absolute inset-0 rounded-full animate-ping bg-indigo-500/30"></div>}
                    </div>
                    
                    <div className="flex-1 pt-1">
                      <div className="flex items-center gap-2">
                        <h4 className={`text-sm font-black transition-colors ${isCurrent ? 'text-indigo-600' : isSkipped ? 'text-slate-300' : 'text-slate-700'}`}>
                          {step.label}
                        </h4>
                        {isCurrent && <span className="px-1.5 py-0.5 bg-indigo-100 text-indigo-600 text-[8px] font-black rounded-md uppercase tracking-widest animate-pulse">目前階段</span>}
                        {isSkipped && <span className="text-[9px] font-bold text-slate-300 italic">(已跳過)</span>}
                      </div>
                      <p className={`text-xs mt-0.5 font-bold ${isCurrent ? 'text-slate-500' : 'text-slate-400'} leading-relaxed`}>
                        {step.desc}
                      </p>
                    </div>
                  </div>
        ...
        {recentSubmissions.length > 0 ? (
          <div className="overflow-x-auto pb-4">
            <table className="w-full text-left whitespace-nowrap border-separate" style={{ borderSpacing: 0 }}>
              <thead>
                <tr>
                  <th className="bg-slate-50 px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest rounded-l-2xl border-y border-l border-slate-200">單號</th>
                  <th className="bg-slate-50 px-4 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-y border-slate-200">部門</th>
                  <th className="bg-slate-50 px-4 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-y border-slate-200">類型</th>
                  <th className="bg-slate-50 px-4 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-y border-slate-200">類別</th>
                  <th className="bg-slate-50 px-4 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-y border-slate-200">時間</th>
                  <th className="bg-slate-50 px-4 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-y border-slate-200">時數</th>
                  <th className="bg-slate-50 px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest rounded-r-2xl border-y border-r border-slate-200 text-right">狀態/操作</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                <tr className="h-2"></tr>
                  ...
                  <tr key={r.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-4 py-4 font-bold text-[11px] text-slate-600 border-b border-slate-100">
                      {r.startDate === r.endDate ? (
                        `${r.startDate} ${r.startHour}:${r.startMin}`
                      ) : (
                    ...
                    <td className="px-6 py-4 border-b border-slate-100 text-right">
                      <div className="flex items-center justify-end gap-3">
                        <button onClick={() => onShowFlow(r)} className="px-3 py-1.5 text-indigo-500 bg-indigo-50 hover:bg-indigo-100 rounded-md text-[10px] font-black transition-colors flex items-center gap-1"><GitBranch size={12}/> 流程</button>
                        ...
                        {r.status === 'approved' && (
                          <button onClick={() => setCancelTarget(r)} className="px-3 py-1.5 text-slate-500 bg-slate-50 hover:bg-slate-100 rounded-md text-[10px] font-black transition-colors">抽單</button>
                        )}
                      </div>
                    </td>
                  </tr>
        ...
        {recentSubmissions.length > 0 ? (
          <div className="overflow-x-auto pb-4">
            <table className="w-full text-left whitespace-nowrap border-separate" style={{ borderSpacing: 0 }}>
              <thead>
                <tr>
                  <th className="bg-slate-50 px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest rounded-l-2xl border-y border-l border-slate-200">單號 / 附件</th>
                  <th className="bg-slate-50 px-4 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-y border-slate-200">部門</th>
                  <th className="bg-slate-50 px-4 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-y border-slate-200">假別</th>
                  <th className="bg-slate-50 px-4 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-y border-slate-200">時段</th>
                  <th className="bg-slate-50 px-4 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-y border-slate-200">時數</th>
                  <th className="bg-slate-50 px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest rounded-r-2xl border-y border-r border-slate-200 text-right">狀態/操作</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                ...
                {recentSubmissions.slice(0, 10).map(r => (
                  <tr key={r.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-6 py-4 font-mono font-bold text-slate-600 border-b border-slate-100">
                      <div>{r.serialId}</div>
                      {r.attachmentName && (
                    ...
                    <td className="px-4 py-4 font-bold text-slate-700 border-b border-slate-100">{r.dept || '未設定'}</td>
                    <td className="px-4 py-4 font-black text-[11px] text-slate-700 border-b border-slate-100">{LEAVE_CATEGORIES.find(c => c.id === r.category)?.label || '未設定'}</td>
                    <td className="px-4 py-4 font-bold text-[11px] text-slate-600 border-b border-slate-100">
                      {r.startDate === r.endDate ? (
                        `${r.startDate} ${r.startHour}:${r.startMin}`
                      ) : (
            ...
            searchResults.map(r => (
              <div key={r.id} className="bg-slate-50 p-6 rounded-2xl border hover:border-fuchsia-300 transition-all shadow-sm">
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-[1fr_1fr_1.5fr_1fr_2.5fr_1fr_auto] gap-4 items-center w-full">
                  ...
                  <div className="flex justify-end col-span-2 sm:col-span-3 md:col-span-1 gap-2 items-center">
                    <button onClick={() => onShowFlow(r)} className="p-2 text-indigo-500 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors" title="檢視路徑"><GitBranch size={16}/></button>
                    <StatusBadge status={r.status} />
                  </div>
                </div>
              </div>
            ...
            <div key={r.id} onClick={() => setSelectedId(r.id)} className={`p-5 rounded-2xl border transition-all cursor-pointer text-left ${selectedId === r.id ? 'bg-amber-50 border-amber-300 ring-2 ring-inset ring-amber-200' : 'bg-slate-50 hover:bg-white hover:border-amber-200 border-slate-100'}`}>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-[auto_1.5fr_1fr_2fr_1fr_2fr_auto] gap-4 items-center w-full">
                ...
                <div className="flex justify-end items-center col-span-2 sm:col-span-3 md:col-span-1 gap-2">
                  <button onClick={(e) => { e.stopPropagation(); onShowFlow(r); }} className="p-2 text-indigo-500 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors" title="檢視路徑"><GitBranch size={16}/></button>
                  <StatusBadge status={r.status} />
                </div>
              </div>
            </div>
...
const App = () => {
  ...
  const [viewingFlow, setViewingFlow] = useState(null); // 新增：當前檢視流程的單據
  ...
  return (
    <div className="h-screen w-full bg-slate-50 flex text-left font-sans text-slate-900 overflow-hidden">
      ...
      {/* 新增：流程細節 Modal */}
      {viewingFlow && (
        <WorkflowModal record={viewingFlow} employees={employees} onClose={() => setViewingFlow(null)} />
      )}
